...
_Composite evaluators_ 用于把多个 evaluator 分数组合成一个单独的[score](/langsmith/evaluation-concepts#evaluator-outputs)。当你希望评估 application 的多个方面，并把结果合并为单一结果时，这会非常有用。

本指南介绍如何通过 [LangSmith SDK](https://reference.langchain.com/python/langsmith/observability/sdk) 设置一个评估流程：它先使用多个 evaluators，再通过自定义聚合函数将它们的分数合并起来。

<Note> Requires langsmith>=0.4.29 </Note>

<Tip>
如果你想在 LangSmith UI 中创建 composite evaluators，请参阅[如何创建 composite evaluator（UI）](/langsmith/composite-evaluators-ui)。
</Tip>

## 1. 在数据集上配置 evaluators

先配置你的 evaluators。在本例中，应用会根据博客导语生成一条 tweet，并使用三个 evaluators，即 summary、tone 和 formatting，对输出进行评估。

如果你已经有自己的数据集，并且其中已经配置好了 evaluators，可以跳过这一步。
...
## 2. 创建 composite feedback

创建 composite feedback，通过你的自定义函数聚合各个 evaluator 的分数。本例使用加权平均的方式，对单个 evaluator 分数进行聚合。
