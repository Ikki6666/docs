...
本指南假设你已经了解什么是 double-texting。如果还不了解，可以先阅读 [double-texting conceptual guide](/langsmith/double-texting)。

本指南介绍 double texting 中的 `enqueue` 选项。它会把中断请求加入队列，并按 client 接收到的顺序依次执行。下面会给出一个使用 `enqueue` 的快速示例。

在 [Agent Server](/langsmith/agent-server) 中创建 runs 时，`enqueue` 是默认的 double texting（multi-tasking）策略。

## 设置

首先，我们先定义一个辅助函数，用来打印 JS 和 CURL 的模型输出（如果你使用的是 Python，可以跳过这一步）：
...
接着，导入所需依赖，并初始化 client、assistant 和 thread。
...
现在我们来启动两个 runs，其中第二个 run 会以 `"enqueue"` 的 multitask strategy 中断第一个 run：
