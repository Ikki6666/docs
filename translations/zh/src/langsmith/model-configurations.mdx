...
<Note>
如果某个 provider 在整个组织范围内已于 **Settings** > **Model providers** 中被禁用，那么它在这个表中会显示为锁定状态，且不能在单个工作区中重新启用。请参阅[组织范围的 provider 控制](#组织范围的-provider-控制)。
</Note>

## 组织范围的 provider 控制

provider 的可用性由两个层级共同控制：

- **Organization**（组织管理员）：为**整个组织**开启或关闭 provider，影响每个[工作区](/langsmith/administration-overview#workspaces)和每个功能。在 **Settings** > **Model providers** 中管理。
- **Workspace**（工作区管理员）：即上面的 [Feature Access](#功能访问) 表，用于控制单个工作区内**按功能划分**的 provider 和 model 可用性。

组织级设置优先级更高。若某个 provider 在组织级被禁用，那么它会在所有工作区和所有功能中都不可用，并在各工作区的 Feature Access 表中显示为**锁定**，工作区管理员可以看到它，但不能重新启用。工作区只能在组织允许的 provider 范围内进行选择。

管理组织范围内的 provider 需要 [Organization Admin](/langsmith/rbac#organization-admin) 角色，也就是 `organization:manage` 权限，并且仅适用于 LangSmith [Cloud](/langsmith/cloud) 和以组织形式使用的 [Self-hosted](/langsmith/self-hosted) 部署，不适用于个人账户。

### 为组织禁用 provider

1. 前往 **Settings** 下的 [Model providers](https://smith.langchain.com/settings/model-providers) 页面。
1. 关闭某个 provider，即可在整个组织范围内禁用它；重新打开即可恢复。
