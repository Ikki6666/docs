...
<Note>
在自托管 LangSmith 上，运维人员必须先在 LangSmith Helm chart 中启用 Engine，然后才能执行上述任一步骤。请参阅[启用 Engine](/langsmith/deploy-self-hosted-full-platform#enable-engine) 和[自托管上的 Engine](/langsmith/engine-self-hosted)。
</Note>
...
| **Recurring scans** | 按照动态调度自动运行 | 10-15 LCUs |
...
<Steps>
  ...
  <Step title="Connect a code repository (optional)">
    虽然是可选项，但推荐连接一个代码仓库。Engine 会读取你的源代码，定位失败 trace 背后的代码路径，让修复建议扎根于实际实现，并可直接从 issue 创建 pull request。在 **Connect your agent's code repository** 下的 **GitHub Repository** 字段中选择一个仓库。这里只会显示 GitHub app 有权限访问的仓库。点击 **Manage app access →** 可更新权限。关于 GitHub App 的设置和组织审批，请参阅[将 Engine 连接到 GitHub](/langsmith/engine-github)。如果要为 Engine 提供额外的项目上下文，请在 **Context Hub repository** 字段中选择一个仓库。你可以在 [**Engine Settings**](#configure-engine) 面板中随时更新任一仓库。
  </Step>
...
<Note>
**范围限制：**范围筛选器只接受 run 名称和 metadata 条件。你无法按 feedback key、evaluator 名称或分数阈值来限定 Engine 的扫描范围。如果你希望 Engine 聚焦于某个特定 evaluator 低分对应的 trace，请使用 [**Preferences** 和 **Agent overview** 设置](#configure-engine) 来告诉 Engine 应优先处理什么。Engine 本身已会自动纳入所有 feedback 信号。参见 [Engine 如何采样并确定 trace 优先级](#how-engine-samples-and-prioritizes-traces)。
</Note>
...
- **Focus on specific traces**：按 run 名称或 metadata 把 Engine 的关注范围收窄到一部分 run。编辑会自动保存，并在下一次扫描时生效。范围条件只接受 run 名称和 metadata，你无法按 feedback key 或分数进行过滤。详见 [Focus on specific traces](#focus-on-specific-traces)。
...
- **Code repository**：连接或更新 GitHub 仓库，让 agent 在诊断问题时可以引用源代码。可选设置 **Subfolder** 和 **Branch**（默认为仓库的默认分支）。关于设置方式，请参阅[将 Engine 连接到 GitHub](/langsmith/engine-github)。
- **Context repository**：连接一个 Context Hub 仓库，让 Engine 可以对 instructions、docs 和已关联的 skills 提出修复建议。
- **Pause**：Engine 按照为平衡成本和性能而调整的动态调度来扫描 trace。点击 **Pause** 可在不删除已有问题的情况下停止扫描，点击 **Resume** 可恢复扫描。
- **Delete all issues**：此操作无法撤销。所有问题和设置都会被永久删除。

## See also

- [Engine](/langsmith/engine-overview)：产品概览，以及 Engine 在开发生命周期中的定位。
- [将 Engine 连接到 GitHub](/langsmith/engine-github)：在 LangSmith Cloud 中连接仓库，或为自托管部署创建并配置你自己的 GitHub App。
