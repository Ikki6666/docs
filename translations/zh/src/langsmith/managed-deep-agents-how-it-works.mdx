...
当你声明 [identity](/langsmith/managed-deep-agents-identity) 时，Managed Deep Agents 会把 threads 和 store namespaces 限定到已认证的 actor 或 tenant 范围内，确保调用方无法打开彼此的对话或 memory。若未声明 identity，deployment 会使用默认的共享边界。
...
- [Identity](/langsmith/managed-deep-agents-identity)：认证调用方，并对 threads 和 memory 做作用域隔离。
