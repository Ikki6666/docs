...
可以通过 SDK 使用 [aevaluate()](https://docs.smith.langchain.com/reference/python/evaluation/langsmith.evaluation._arunner.aevaluate) 异步运行评估。它接收与 [evaluate()](https://docs.smith.langchain.com/reference/python/evaluation/langsmith.evaluation._runner.evaluate) 相同的参数，但要求应用函数本身是异步的。更多信息请参阅[如何使用 `evaluate()` 函数](/langsmith/evaluate-llm-application)。

<Info>
本指南仅适用于使用 Python SDK 的场景。在 JS/TS 中，`evaluate()` 函数本身已经是异步的。更多信息请参阅[评估 LLM 应用](/langsmith/evaluate-llm-application)。
</Info>
...
* [同步运行评估](/langsmith/evaluate-llm-application)
