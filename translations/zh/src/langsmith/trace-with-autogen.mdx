...
LangSmith 可以通过 OpenTelemetry instrumentation 捕获 [AutoGen](https://microsoft.github.io/autogen/stable/) 生成的 traces。本指南介绍如何自动采集 AutoGen 多智能体对话中的 traces，并将其发送到 LangSmith 用于监控和分析。
...
在你的 AutoGen 应用中，配置 LangSmith OpenTelemetry 集成，并同时启用 OpenAI instrumentor：
...
完成配置后，你的 AutoGen 应用会自动将 traces 发送到 LangSmith。为了获得完整 tracing 覆盖，请把 tracer provider 传给 runtime：
...
你可以把 AutoGen tracing 与其他 OpenTelemetry instrumentors 组合使用：
