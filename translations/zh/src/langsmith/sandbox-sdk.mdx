...
这两个值都必须是 60 的倍数（按分钟粒度）。完整生命周期如下：

```
running ──(idle for idle_ttl_seconds)──▶ stopped ──(delete_after_stop_seconds)──▶ deleted
```

你也可以显式调用 `stop_sandbox` / `stopSandbox`，这同样会填充 `stopped_at` 并启动删除计时器。
...
- **Session TTL（已完成命令）**：命令结束后，其会话会在内存中保留一段 TTL 时间。在此期间你可以重新连接并取回输出。TTL 到期后，会话会被清理。
- **Idle timeout（运行中的命令）**：如果运行中的命令没有任何客户端连接，就会在空闲超时后被终止（默认 5 分钟）。每当客户端连接时，空闲计时器都会重置。设置为 `-1` 可禁用空闲超时。
