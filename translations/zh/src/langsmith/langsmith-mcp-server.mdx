...
LangSmith MCP Server 提供了一个通过 HTTP 访问的托管版本，因此你无需自己运行 server。

- **URL:** `https://langsmith-mcp-server.onrender.com/mcp`
- **认证方式:** 在 `LANGSMITH-API-KEY` 请求头中发送你的 [LangSmith API key](/langsmith/create-account-api-key)

<Note>
该托管实例适用于 [LangSmith Cloud](/langsmith/deploy-to-cloud)。如果你使用的是 [self-hosted LangSmith](/langsmith/self-hosted)，请自行运行 server，并将其指向你的实例 endpoint（参见 [Docker deployment](#docker-deployment-http-streamable)）。
</Note>
...
可选请求头：`LANGSMITH-WORKSPACE-ID`、`LANGSMITH-ENDPOINT`（含义与[环境变量](#environment-variables)一致）。
