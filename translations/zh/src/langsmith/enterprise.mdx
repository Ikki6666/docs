---
title: 面向企业的 LangSmith
sidebarTitle: 企业功能
description: 面向企业用户的部署选项、访问控制、数据隐私、成本控制和安全合规。
mode: wide
icon: "building-skyscraper"
---

这个页面是企业团队的参考入口，汇总了组织常见的关键信息，例如[部署选项](#deployment-options)、[访问控制](#access-control)、[数据隐私](#data-privacy-and-pii)和[成本控制](#cost-controls-and-usage)。

<Callout>
如果你想了解企业版[定价](/langsmith/pricing-plans)或准备开始使用，请[联系销售团队](https://www.langchain.com/contact-sales)。
</Callout>

## Deployment options

选择最适合你的基础设施和数据驻留要求的 LangSmith 托管方式。

<CardGroup cols={3}>
  <Card title="Cloud" icon="cloud" href="/langsmith/cloud">
    在 LangSmith 托管云中运行 LangSmith，并使用美国或欧盟数据驻留。
  </Card>
  <Card title="Hybrid" icon="topology-complex" href="/langsmith/hybrid">
    让控制平面运行在 LangSmith 云中，把数据平面放在你自己的 VPC 中，实现完整的数据隔离。
  </Card>
  <Card title="Self-hosted" icon="server-2" href="/langsmith/self-hosted">
    使用 Docker Compose 或 Kubernetes，在你自己的基础设施中完整部署 LangSmith。
  </Card>
</CardGroup>

## User management

在整个组织中管理用户并自动化配置流程。

<CardGroup cols={2}>
  <Card title="User management" icon="users" href="/langsmith/user-management">
    邀请用户、分配角色，并配置 SCIM 以实现自动的开通和停用。
  </Card>
  <Card title="SSO & JIT provisioning" icon="login" href="/langsmith/authentication-methods">
    为你的身份提供方配置 SAML 或 OIDC 单点登录，以及即时开通（JIT）用户配置。
  </Card>
  <Card title="Organization setup" icon="building" href="/langsmith/set-up-hierarchy">
    创建并配置组织、工作区以及企业内部的用户层级。
  </Card>
  <Card title="Manage by API" icon="terminal-2" href="/langsmith/manage-organization-by-api">
    通过 API 以编程方式管理用户、配置安全设置并执行组织管理。
  </Card>
</CardGroup>

## Access control

控制组织中谁可以访问哪些内容。

<CardGroup cols={2}>
  <Card title="Role-based access control (RBAC)" icon="shield-lock" href="/langsmith/rbac">
    使用内置角色或自定义角色定义工作区级别的权限。仅企业版方案提供。
  </Card>
  <Card title="Attribute-based access control (ABAC)" icon="tag" href="/langsmith/abac">
    应用细粒度的基于标签的访问策略来限制资源访问，包括阻止特定用户访问包含 PII 的数据。
  </Card>
  <Card title="Workload isolation" icon="layout-sidebar" href="/langsmith/workload-isolation">
    使用多工作区模型隔离团队、建立信任边界并分离环境。
  </Card>
  <Card title="Resource tags" icon="tag" href="/langsmith/set-up-resource-tags">
    为资源打标签，用于 ABAC 策略，以及组织 `dev`、`staging` 和 `prod` 等环境。
  </Card>
</CardGroup>

## Data privacy and PII

控制敏感数据的存储和访问方式。

<CardGroup cols={2}>
  <Card title="Data storage & privacy" icon="database" href="/langsmith/data-storage-and-privacy">
    了解 LangSmith 存储哪些数据、加密如何工作，以及如何关闭遥测和 tracing。
  </Card>
  <Card title="PII controls with ABAC" icon="eye-off" href="/langsmith/abac">
    使用 ABAC 拒绝策略限制对包含个人身份信息的 traces 和 datasets 的访问。
  </Card>
</CardGroup>

## Data retention & cleanup

配置数据保留时长以及删除方式。

<CardGroup cols={2}>
  <Card title="Data purging for compliance" icon="trash" href="/langsmith/data-purging-compliance">
    设置自定义保留周期、按元数据删除 traces，并满足删除合规要求。
  </Card>
  <Card title="Data retention settings" icon="clock" href="/langsmith/usage-and-billing#data-retention">
    了解基础与扩展保留层级、自动升级机制，以及数据保留如何影响计费。
  </Card>
</CardGroup>

## Cost controls and usage

在整个组织内跟踪并限制支出。

<CardGroup cols={2}>
  <Card title="Billing & spend limits" icon="credit-card" href="/langsmith/billing">
    设置每月使用量上限、跟踪预付合同使用情况，并优化 tracing 支出。
  </Card>
  <Card title="Granular usage reporting" icon="chart-bar" href="/langsmith/granular-usage">
    按工作区、项目、用户或 API 密钥拆分 trace 使用量，以便将成本归因到各个团队。
  </Card>
</CardGroup>

## Security & compliance

查看 LangSmith 的安全态势和合规认证。

<CardGroup cols={2}>
  <Card title="Shared responsibility model" icon="shield-check" href="/langsmith/shared-responsibility-model">
    查看 LangChain 与你的组织之间的安全职责划分。LangSmith 拥有 SOC 2 Type II、HIPAA 和 GDPR 认证。
  </Card>
  <Card title="Scalability & resilience" icon="chart-arrows-vertical" href="/langsmith/scalability-and-resilience">
    查看 SLA 保证、灾难恢复策略和高可用配置。
  </Card>
</CardGroup>
