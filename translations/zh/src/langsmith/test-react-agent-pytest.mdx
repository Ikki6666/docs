---
title: 使用 Pytest/Vitest 和 LangSmith 测试 ReAct agent
sidebarTitle: 使用 Pytest/Vitest 和 LangSmith 测试 ReAct agent
---
...
本教程将向你展示如何使用 LangSmith 与常见测试工具（Pytest、Vitest 和 Jest）的集成来评估 LLM 应用。我们会创建一个能够回答上市公司股票相关问题的 ReAct agent，并为它编写一套完整的测试。

## 设置

本教程使用 [LangGraph](https://langchain-ai.github.io/langgraph/tutorials/introduction/) 进行 agent 编排，使用 [OpenAI 的 GPT-4o](https://platform.openai.com/docs/models#gpt-4o)、[Tavily](https://tavily.com/) 做搜索、[E2B](https://e2b.dev/) 的代码解释器，以及 [Polygon](https://polygon.io/stocks) 获取股票数据。不过只要做少量修改，也可以适配到其他框架、模型和工具。Tavily、E2B 和 Polygon 都可以免费注册。
...
为了定义我们的 ReAct agent，我们将使用 LangGraph/LangGraph.js 进行编排，使用 LangChain 来提供 LLM 和工具。

### 定义工具

首先，我们来定义 agent 要使用的工具。这里会有 3 个工具：
...
现在我们已经定义好了所有工具，可以使用 @[`create_agent`] 来创建 agent。
...
现在 agent 已经定义好了，我们来写几个测试，确保基本功能正常。在本教程中，我们将测试 agent 的工具调用能力是否正常、是否知道忽略无关问题，以及是否能够回答需要用到所有工具的复杂问题。

首先，需要创建一个测试文件，并在文件顶部加入所需的导入。
...
第一个测试很简单，用来检查 agent 在面对无关查询时不会调用工具。
...
在工具调用测试中，我们要验证 agent 是否调用了正确的工具，并传入了正确的参数。
...
有些工具调用比另一些更容易测试。对于 ticker 查询，我们可以直接断言是否搜索了正确的 ticker。对于代码工具来说，工具的输入和输出约束要弱得多，而且得到正确答案的路径也有很多。这种情况下，更简单的做法是运行完整 agent，并断言它既调用了代码工具，也得到了正确答案。
...
我们将通过运行一次 LLM-as-a-judge 评估，来确保 agent 的回答确实以搜索结果为依据。为了让这次 LLM-as-a-judge 调用在 LangSmith 中与 agent 的 trace 分开记录，我们会在 Python 中使用 LangSmith 提供的 `trace_feedback` 上下文管理器，在 JS/TS 中使用 `wrapEvaluator` 函数。
...
如果你使用的是 Vitest 或 Jest，在配置好相应文件之后，可以使用以下命令运行测试：
...
不要忘记把 Vitest 和 Jest 的配置文件也加入到你的项目中。
