...
- **计算（Compute）**：Deployment 在资源 provision 期间所使用的 vCPU 和内存，以 LangChain Compute Units（LCU）计量。[Serverless](/langsmith/cloud-platform-features#serverless) deployment 在一段时间不活跃后可以[缩容到零（beta）](/langsmith/cloud-platform-features#serverless)，因此只有在其缩容之后，计算费用才会停止。[Dedicated](/langsmith/cloud-platform-features#dedicated) deployment 始终在线，会持续消耗计算资源。
- **存储（Storage）**：Deployment 用于持久化状态的数据库存储，以 LangChain Storage Units（LSU）计量。

关于当前的 LCU 和 LSU 费率，以及如何估算 deployment 成本，请参阅[定价页面](https://www.langchain.com/pricing），其中包含 deployment 成本计算器。

<Note>
这种基于用量的计费模式取代了此前的按次运行和 uptime 定价。现有客户在 2026 年 10 月 1 日之前仍保持当前定价，之后会切换到新模式。缩容到零仅适用于采用新定价的 deployment。在功能逐步推出期间，Serverless deployment 缩容到零之前的不活跃时间窗口可能会有变化。如对过渡有疑问，请通过 [support.langchain.com](https://support.langchain.com) 联系支持团队。
</Note>
