...
你可以使用 Python 和 TypeScript SDK 以编程方式管理数据集，包括创建、更新和删除数据集，以及向其中添加 examples。

## 创建数据集

### 从值列表创建数据集

使用 client 创建数据集时，最灵活的方式是从一组输入，以及可选输出，来创建 examples。下面是一个示例。

请注意，你可以为每个 example 添加任意元数据，例如备注或来源。元数据会以字典形式存储。

<Check>
如果你需要创建大量 examples，建议使用 `create_examples`/`createExamples`，通过一次请求批量创建多个 examples。如果只创建单个 example，可以使用 `create_example`/`createExample`。
</Check>
...
若要根据 trace 中的 runs，也就是 spans，创建数据集，也可以使用相同的方法。关于如何拉取和过滤 runs，更多示例请参阅[导出 traces](/langsmith/export-traces)。下面是一个示例：
...
本节演示如何通过上传 CSV 文件来创建数据集。

首先，请确保 CSV 文件的格式正确，包含能够表示输入键和输出键的列。这些键会在上传过程中用于正确映射数据。你还可以为数据集指定可选的名称和描述，否则会使用文件名作为数据集名称，并且不会附带描述。
...
### 从 pandas DataFrame 创建数据集，仅 Python

Python client 还提供了一个便捷方法，可直接从 pandas dataframe 上传数据集。
...
你可以通过 Python 和 TypeScript SDK 中的 `list_datasets`/`listDatasets` 以编程方式获取 LangSmith 数据集。下面是一些常见调用方式。

<Info>
在运行下面的代码片段前，请先初始化 client。
</Info>
...
你可以通过 Python 和 TypeScript SDK 中的 `list_examples`/`listExamples` 以编程方式获取 LangSmith examples。下面是一些常见调用方式。
...
你也可以按 dataset 名称过滤，但它必须与要查询的数据集名称完全一致：
...
你也可以按元数据过滤 examples。下面的示例查询包含特定元数据键值对的 examples。底层逻辑会检查该 example 的元数据是否包含你指定的键值对。

例如，如果某个 example 的 metadata 是 `{"foo": "bar", "baz": "qux"}`，那么 `{foo: bar}` 和 `{baz: qux}` 都会匹配，`{foo: bar, baz: qux}` 也会匹配。
...
和你使用结构化过滤查询语言来[获取 runs](/langsmith/export-traces#use-filter-query-language)类似，也可以用它来获取 examples。

<Note>
该能力目前仅适用于 Python SDK v0.1.83 及以上版本，以及 TypeScript SDK v0.1.35 及以上版本。

另外，结构化过滤查询语言目前只支持 `metadata` 字段。
</Note>

你可以使用 `has` 运算符来获取 metadata 字段包含指定键值对的 examples，也可以使用 `exists` 运算符来获取 metadata 字段中包含某个键的 examples。此外，还可以通过 `and` 运算符串联多个过滤条件，并通过 `not` 运算符对条件取反。
...
你可以通过 Python 和 TypeScript SDK 中的 `update_example`/`updateExample` 以编程方式更新 LangSmith examples。下面是一个示例。
...
你也可以通过 Python 和 TypeScript SDK 中的 `update_examples`/`updateExamples` 在一次请求中以编程方式批量更新多个 examples。下面是一个示例。
