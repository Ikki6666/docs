...
本指南默认你已经了解什么是 double-texting。你可以先阅读 [double-texting 概念指南](/langsmith/double-texting)。

本指南介绍 double texting 的 `interrupt` 选项。它会中断图的上一次运行，并使用新的双发消息启动一次新的运行。这个选项不会删除第一次运行，而是会将它保留在数据库中，并把其状态设置为 `interrupted`。下面是一个使用 `interrupt` 选项的简短示例。

## 设置

首先，我们定义一个简单的辅助函数，用于打印 JS 和 CURL 的模型输出（如果你使用 Python，可以跳过这一步）：
...
现在，导入所需包并初始化 client、assistant 和 thread。
...
可以看到，线程中既保留了第一次运行的部分数据，也包含了第二次运行的数据。
