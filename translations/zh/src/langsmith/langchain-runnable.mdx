---
title: 如何评估一个 runnable
sidebarTitle: 评估一个 runnable
---
...
`langchain` 的 [`Runnable`](https://reference.langchain.com/python/langchain_core/runnables/) 对象，例如聊天模型、retriever、chain 等，可以直接传给 `evaluate()` / `aevaluate()`。

## 设置

先定义一个简单的 chain 用于评估。首先安装所需依赖：
...
要评估这个 chain，可以直接把它传给 `evaluate()` / `aevaluate()`。注意，chain 的输入变量必须与示例输入的键一致。在这里，示例输入应为 `{"text": "..."}` 这种形式。
...
系统会为 runnable 的每个输出正确记录 trace。

![Runnable Evaluation](/langsmith/images/runnable-eval.png)

## 相关内容

* [如何评估一个 `langgraph` graph](/langsmith/evaluate-on-intermediate-steps)
