---
title: 在 traced function 中访问当前 run（span）
sidebarTitle: 在 traced function 中访问当前 run（span）
---

在某些情况下，你会希望在 traced function 中访问当前 run（span）。这对于提取当前 run 中的 UUID、tags 或其他信息很有帮助。

你可以分别在 Python 或 TypeScript SDK 中调用 `get_current_run_tree` / `getCurrentRunTree` 函数来访问当前 run。

如需查看 `RunTree` 对象上所有可用属性的完整列表，请参阅[此参考文档](/langsmith/run-data-format)。
