---
title: 如何在同一个 thread 上运行多个 agents
sidebarTitle: 在同一个 thread 上运行多个 agents
---
在 LangSmith Deployment 中，thread 并不会显式绑定到某一个特定 agent。
这意味着你可以在同一个 thread 上运行多个 agents，从而让另一个 agent 基于前一个 agent 的执行进度继续往下走。

在这个示例中，我们会创建两个 agents，然后在同一个 thread 上分别调用它们。
你会看到，第二个 agent 会把第一个 agent 在该 thread 中生成的 [checkpoint](/oss/langgraph/checkpointers#checkpoints) 作为上下文来响应。
...
现在我们先在这个 thread 上运行 OpenAI assistant。
...
现在，我们可以在 default assistant 上继续运行，并看到第二个 assistant 已经知道前一个问题，能够继续回答 “and you?” 这个追问：
