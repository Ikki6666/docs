...
LangSmith CLI 是一个用于查询和管理 LangSmith 数据的命令行工具。它既适用于开发者，也适用于 AI 编码代理。默认输出为适合人类阅读的表格；如果需要用于脚本处理，可以使用 `--format json`。当你需要以可脚本化的方式访问 LangSmith 数据时，例如批量导出、自动化处理，或者给编码代理直接访问你的 [traces、runs 和 datasets](/langsmith/observability-concepts)，就可以使用它。
