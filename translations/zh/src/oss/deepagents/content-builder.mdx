...
title: 构建内容创作 agent
sidebarTitle: 内容创作
description: 构建一个包含品牌 memory、skills、subagents 和图像生成能力的内容写作 agent
---
...
本指南演示如何使用 [Deep Agents](/oss/deepagents) 从零构建一个内容写作 agent。
...
- [Long-term memory](/oss/deepagents/memory)：用于长期保存上下文
- [Skills](/oss/deepagents/skills)：用于封装可复用能力
- [Subagents](/oss/deepagents/subagents)：用于把任务委派给专门的子代理
- [Filesystem backends](/oss/deepagents/backends)：用于文件读写
- 自定义 [tools](/oss/langchain/tools)：用于搜索和图像生成
...
这个示例把行为分散在三类文件里：memory、skills 和 subagent 定义。

<Steps>
<Step title="添加 AGENTS.md">

在项目根目录创建 `AGENTS.md`。
之后创建 agent 时，如果把这个文件作为 [memory](/oss/deepagents/memory) 参数的一部分传入，它会被加载进 system prompt，这样品牌语气和研究要求就会对每次运行生效。
...
如果你想让这个 agent 遵循你自己的语气、内容支柱和格式规则，就修改 `AGENTS.md` 里的文本。
...
然后添加下面这段文本，它描述了一个 `researcher` subagent，包含基于 Tavily 的 `web_search` tool、一个 Haiku model id，以及在从主 agent 委派时把结果保存到你指定路径的说明：
...
之后在创建 deep agent 时，会把这个文件作为参数传入。
...
创建一个 `skills/` 目录。每个 skill 都是一个文件夹，里面包含一个 `SKILL.md` 文件，文件中有 YAML frontmatter（`name`、`description`）和这个 skill 的指令。

创建 `skills/blog-post/SKILL.md`，并把下面的文本复制进去。它包含如何撰写长文、优化 SEO，以及生成封面图像的信息。
...
接着，创建 `skills/social-media/SKILL.md`，并把下面的文本复制进去。它包含如何起草社交媒体帖子以及生成配图的信息：
