...
<Tip>
要生成可供编码 agent 通过 [`AGENTS.md`](https://agents.md/) 发现的仓库 wiki，请参见 [OpenWiki](/oss/openwiki/overview)。
</Tip>
...
对于可供编码 agent 通过 [`AGENTS.md`](https://agents.md/) 发现的生成式仓库 wiki，请参见 [OpenWiki](/oss/openwiki/overview)。
...
情景记忆存储过去经历的记录：发生了什么、发生的顺序以及结果是什么。与语义记忆（存储在“AGENTS.md”等文件中的事实和偏好）不同，情景记忆保留了完整的对话上下文，因此代理可以回忆“如何”解决问题，而不仅仅是从中“学到什么”。要为编码 agent 生成和维护仓库级别的 wiki，请参见 [OpenWiki](/oss/openwiki/overview)。
...
- [OpenWiki](/oss/openwiki/overview)：生成并维护可供编码 agent 通过 `AGENTS.md` 发现的仓库 wiki
- [Backends](/oss/deepagents/backends)：选择 memory 文件的存储位置
- [Context engineering](/oss/deepagents/context-engineering)：短期记忆、offloading 和摘要
- [Skills](/oss/deepagents/skills)：按需程序性记忆
