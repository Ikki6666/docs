...
| `中间件` | `AgentMiddleware[]` |选修的。用于自定义行为、日志记录或速率限制的附加中间件。<br></br>不继承自主代理。它会合并进[同步子代理堆栈](/oss/deepagents/customization#synchronous-subagent-stack)：`.name` 与默认项匹配的实例会原位替换默认项，其余条目会落在最后一个核心 middleware 之后、profile、prompt caching 和 memory 之前。请参见[覆盖默认 middleware 实例](/oss/deepagents/customization#override-a-default-middleware-instance)。例如，你可以在这里加入带 `tools` allowlist 的 @[`FilesystemMiddleware`] 实例，以便把子代理的文件系统工具限制得与主代理独立。更多信息请参见[虚拟文件系统访问](/oss/deepagents/overview#virtual-filesystem-access)下的“限制文件系统工具”。 |
