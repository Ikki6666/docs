...
| `中间件` | `列表[中间件]` |选修的。用于自定义行为、日志记录或速率限制的附加中间件。<br></br>不继承自主代理。它会合并进[默认子代理堆栈](/oss/deepagents/customization#default-stack-synchronous-subagents)：`.name` 与默认项匹配的实例会原位替换默认项，其余条目会落在最后一个核心 middleware 之后、profile、prompt caching 和 memory 之前。请参见[覆盖默认 middleware 实例](/oss/deepagents/customization#override-a-default-middleware-instance)。例如，你可以在这里加入带 `tools` allowlist 的 @[`FilesystemMiddleware`] 实例，以便把子代理的文件系统工具限制得与主代理独立。更多信息请参见[虚拟文件系统访问](/oss/deepagents/overview#virtual-filesystem-access)下的“限制文件系统工具”。 |
...
| `中间件` | `AgentMiddleware[]` |选修的。用于自定义行为、日志记录或速率限制的附加中间件。<br></br>不继承自主代理。它会合并进[默认子代理堆栈](/oss/deepagents/customization#default-stack-synchronous-subagents)：`.name` 与默认项匹配的实例会原位替换默认项，其余条目会落在最后一个核心 middleware 之后、profile、prompt caching 和 memory 之前。请参见[覆盖默认 middleware 实例](/oss/deepagents/customization#override-a-default-middleware-instance)。例如，你可以在这里加入带 `tools` allowlist 的 @[`FilesystemMiddleware`] 实例，以便把子代理的文件系统工具限制得与主代理独立。更多信息请参见[虚拟文件系统访问](/oss/deepagents/overview#virtual-filesystem-access)下的“限制文件系统工具”。 |
...
<Note>
只要代理具有子代理和解释器中间件，动态子代理调度就会默认打开。传递 `CodeInterpreterMiddleware(subagents=False)` 以要求通过正常的 `task` 工具路径进行调度。解释器需要 `langchain-quickjs>=0.2.0` 和 Python `>=3.11`。
</Note>
:::
...
尝试动态子代理的最快方法是使用 `dcode`，这是基于 Deep Agent 构建的 LangChain 终端编码 agent。它附带启用的代码解释器，因此动态子代理可以开箱即用，无需连接任何东西。
...
![LangSmith Runs 视图中，metadata filter 将 lc_agent_name 设为 coordinator](/langsmith/images/deepagents-lc-agent-name-filter.png)
...
- 使用自己的[应用了配置文件覆盖的默认系统提示符](/oss/deepagents/customization#system-prompt)
- 可以使用所有相同的工具
- 使用相同的模型（除非被覆盖）
- 继承主代理的技能（配置技能时）
