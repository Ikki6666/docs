...
async_subagents = [
    AsyncSubAgent(
        ...
        graph_id="researcher",
        # 没有 url → ASGI transport（与部署共置）
    ),
    AsyncSubAgent(
        name="coder",
        description="Coding agent for code generation and review",
        graph_id="coder",
        # url="https://coder-deployment.langsmith.dev"  # 可选：用于远程的 HTTP transport
    ),
]
...
当配置了异步子代理时，包含在 [默认 middleware 栈](/oss/deepagents/customization#default-stack-main-agent) 中的 @[`AsyncSubAgentMiddleware`] 会为 supervisor 提供五个工具：
