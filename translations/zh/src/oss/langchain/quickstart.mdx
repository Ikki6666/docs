...
本快速入门向你展示如何在几分钟内创建一个功能完整的 AI 智能体。

<Tip>
    **使用 AI 编程助手？**

    - 安装 [LangChain 文档 MCP 服务器](/use-these-docs)，让你的智能体访问最新的 LangChain 文档和示例。
    - 安装 [LangChain Skills](https://github.com/langchain-ai/langchain-skills)，提升你的智能体在 LangChain 生态系统任务上的表现。
</Tip>
...
从[任何支持的模型提供商](/oss/integrations/providers/overview)获取 API 密钥（例如 Google Gemini 或 OpenAI）。
...
```
  ...
  <Tab title="Other">
    请参阅支持的[聊天模型集成](/oss/integrations/chat)完整列表。
  </Tab>
</Tabs>

## 构建基本智能体

首先创建一个可以回答问题并调用工具的简单智能体。本示例中的智能体使用所选的语言模型、一个基本的天气函数作为工具，以及一个简单的提示词来引导其行为：
...
当你运行代码并让智能体告诉你旧金山的天气时，智能体使用该输入和可用上下文。
智能体理解你在询问旧金山这座城市的天气，因此使用提供的城市名称调用天气工具。

<Tip>
你可以通过更改模型名称和设置相应的 API 密钥来使用[任何支持的模型](/oss/integrations/providers/overview)。使用 [LangSmith](https://smith.langchain.com) 追踪智能体内部发生的情况。按照[追踪快速入门](/langsmith/trace-with-langchain)进行设置。

我们还建议你设置 [LangSmith Engine](/langsmith/engine)，它会监控你的追踪记录、检测问题并提出修复建议。
</Tip>
...
1. **模型配置**以获得一致的响应
1. **对话记忆**以实现聊天式交互
1. **Deep Agents**以使用内置功能
1. **测试**你的智能体

<Steps>
    <Step title="定义系统提示词">
        系统提示词定义了智能体的角色和行为。保持具体且可操作：
    ...
    <Step title="创建工具">
        ...
        <Tip>
            工具应该有良好的文档：它们的名称、描述和参数名会成为模型提示词的一部分。
            LangChain 的 @[`@tool` 装饰器][@tool]添加元数据，并通过 `ToolRuntime` 参数启用运行时注入。
            在[工具指南](/oss/langchain/tools)中了解更多。
        </Tip>
        :::
        ...
        <Note>
            [Zod](https://zod.dev/) 是一个用于验证和解析预定义模式的库。你可以使用它来定义工具的输入模式，确保智能体只使用正确的参数调用工具。

            或者，你也可以将 `schema` 属性定义为 [JSON 模式](https://json-schema.org/overview/what-is-jsonschema) 对象。请注意，JSON 模式在运行时**不会**被验证。
    ...
    <Step title="配置模型">
        为你的用例设置[语言模型](/oss/langchain/models)的正确参数。例如：
    ...
    <Step title="添加记忆">
        ...
        为你的智能体添加[记忆](/oss/langchain/short-term-memory)以在交互之间维护状态。这使得
        智能体能够记住之前的对话和上下文。
        ...
        <Info>
            ...
            有关更多详情，请参阅[添加和管理记忆](/oss/langgraph/add-memory#manage-short-term-memory)。
        </Info>
    ...
    <Step title="创建并运行智能体">
        ...
        有两种不同的框架可以创建智能体：LangChain 智能体和 Deep Agents。
        LangChain 和 Deep Agents 都为你提供了对工具、记忆等的精细控制。
        两者的主要区别在于 Deep Agents 自带一系列常用的内置功能，如规划、文件系统工具和子智能体。

        当你希望以最少的设置获得最大功能时，使用 Deep Agents；当你需要精细控制时，选择 LangChain 智能体。

        要在这一步中对比两者，请安装 `deepagents` 包：
        ...
        <Warning>
            由于代码使用《了不起的盖茨比》的完整文本调用模型，因此会消耗大量 token。
...
你用 LangChain 构建的大多数有趣应用，都会发起很多次 LLM 调用。随着这些应用变得更复杂，能够检查智能体内部到底发生了什么，就会越来越重要。最好的方式就是使用 [LangSmith](https://smith.langchain.com)。

注册一个 [LangSmith](https://smith.langchain.com) 账号，并设置以下环境变量开始记录 trace：
...
设置完成后，再次运行脚本，然后前往 [LangSmith](https://smith.langchain.com) 查看你的智能体调用过程中发生了什么。

<Tip>
要进一步了解如何用 LangSmith 追踪智能体，请参阅 [LangSmith 文档](/langsmith/trace-with-langchain)。

还建议你同时设置 [LangSmith Engine](/langsmith/engine)，它可以监控 trace、检测问题，并给出修复建议。
</Tip>
...
- **通过上下文处理用户特定信息**
- **在多次交互之间维护对话状态**
- **进行规划、研究和综合分析**（仅限 deep agents）

你可以继续阅读：

- **LangChain agents**：[添加和管理 memory](/oss/langgraph/add-memory#manage-short-term-memory)、[部署到生产环境](/oss/langgraph/deploy)
- **Deep Agents**：[自定义选项](/oss/deepagents/customization)、[持久化 memory](/oss/deepagents/memory)、[部署到生产环境](/oss/langgraph/deploy)
