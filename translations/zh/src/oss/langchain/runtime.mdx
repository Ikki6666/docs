...
def dynamic_system_prompt(request: ModelRequest) -> str:
    ...
    system_prompt = f"你是一个乐于助人的助手。请用 {user_name} 来称呼用户。"
...
:::js
使用 `runtime` 参数可以在中间件内部访问 @[`Runtime`] 对象。
...
const dynamicPromptMiddleware = createMiddleware({
  ...
  beforeModel: (state, runtime) => { // [!code highlight]
    ...
    const systemMsg = `你是一个乐于助人的助手。请用 ${userName} 来称呼用户。`;
    return {
...
:::python
中间件 hook 也可以访问 `runtime.execution_info` 和 `runtime.server_info`：
...
:::js
中间件 hook 也可以访问 `runtime.executionInfo` 和 `runtime.serverInfo`：
