...
LangChain agent 和 LangGraph 这类 agent 框架会在内部调用 `model.bindTools(tools)`。`fakeModel` 会自动处理这一点。绑定后的模型与原始模型共享同一条响应队列和调用记录，因此不需要额外设置：
...
// 调用记录与原始模型共享。通过原始 model 查看即可。
console.log(model.callCount); // 2
```

<Accordion title="完整示例：使用 vitest 测试一个工具调用智能体">
...
了解如何在真实模型提供商 API 上测试你的智能体，请参见 [集成测试](/oss/langchain/test/integration-testing)。
