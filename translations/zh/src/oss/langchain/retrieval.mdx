...
<Note>
    如果你已经有知识库，例如 SQL 数据库、文档数据库、CRM，或者内部文档系统，你**不需要**重建它。你可以：
...
<Card
    ...
    arrow cta="了解更多"
>
    看看如何构建一个基于文档、能够回答问题的 Q&A chatbot，使用的是 Retrieval-Augmented Generation。
    本教程会将 LangChain docs 索引到 vector store，在查询时检索相关 chunk，将它们卸载到 agent filesystem，并委派给 subagents 进行分析。
</Card>
