...
<Tip>
带 progressive disclosure 的 skill 可以看作 [RAG (Retrieval-Augmented Generation)](/oss/langchain/rag) 的一种形式，其中每个 skill 都是一个 retrieval unit，不过它不一定依赖 embeddings 或 keyword search，而是通过用于浏览内容的工具（比如文件操作，或者本教程里的直接查询）。
</Tip>
