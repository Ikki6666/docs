---
title: 使用 `HanaSparqlQAChain` 进行问答
---
## 设置与安装

要使用此功能，请安装 `langchain-hana` 包：

```python
pip install langchain_hana
```

然后，创建到 SAP HANA Cloud 实例的连接。

```python
import os

from dotenv import load_dotenv
from hdbcli import dbapi

# 如果需要则加载环境变量
load_dotenv()

# 建立到 SAP HANA Cloud 的连接
connection = dbapi.connect(
    address=os.environ.get("HANA_DB_ADDRESS"),
    port=os.environ.get("HANA_DB_PORT"),
    user=os.environ.get("HANA_DB_USER"),
    password=os.environ.get("HANA_DB_PASSWORD")
)
```

`HanaSparqlQAChain` 将以下能力串联在一起：

1. **感知 schema 的 SPARQL 生成**
2. **针对 SAP HANA 执行查询**
3. **自然语言答案格式化**

## 初始化

需要准备：

* 一个用于生成和解释查询的 **LLM**
* 一个 **`HanaRdfGraph`**，其中包含连接、`graph_uri` 和 ontology

按照这里的步骤了解如何创建 `HanaRdfGraph` 实例：[HanaRdfGraph](/oss/integrations/graphs/sap_hana_rdf_graph)。

导入 `HanaSparqlQAChain`

```python
from langchain_hana import HanaSparqlQAChain
```

```python
qa_chain = HanaSparqlQAChain.from_llm(
    llm=llm, graph=graph, allow_dangerous_requests=True, verbose=True
)
```

## 流水线概览

1. **SPARQL 生成**
   * 使用 `SPARQL_GENERATION_SELECT_PROMPT`
   * 输入：
     * `schema`（来自 `graph.get_schema` 的 Turtle）
     * `prompt`（用户问题）
2. **查询后处理**
   * 从 llm 输出中提取 SPARQL 代码
   * 如果缺失，则注入 `FROM <graph_uri>`
   * 确保声明了必需的通用前缀（`rdf:`、`rdfs:`、`owl:`、`xsd:`）
3. **执行**
   * 调用 `graph.query(generated_sparql)`
4. **答案生成**
   * 使用 `SPARQL_QA_PROMPT`
   * 输入：
     * `context`（原始查询结果）
     * `prompt`（原始问题）

## 提示模板

### “SPARQL Generation” 提示

`sparql_generation_prompt` 用于引导 LLM 根据用户问题和提供的 schema 生成 SPARQL 查询。

### 回答提示

`qa_prompt` 指导 LLM 仅基于数据库结果生成自然语言答案。

默认提示可在这里查看：[`prompts.py`](https://github.com/SAP/langchain-integration-for-sap-hana-cloud/blob/main/langchain_hana/chains/graph_qa/prompts.py)

## 自定义提示

可以在初始化时覆盖默认值：

```python
qa_chain = HanaSparqlQAChain.from_llm(
    llm=llm,
    graph=graph,
    allow_dangerous_requests=True,
    verbose=True,
    sparql_generation_prompt=YOUR_SPARQL_PROMPT,
    qa_prompt=YOUR_QA_PROMPT
)
```

> - `sparql_generation_prompt` 必须包含输入变量：`["schema", "prompt"]`
> - `qa_prompt` 必须包含输入变量：`["context", "prompt"]`

## 示例：在 “Movies” 知识图谱上进行问答

**前置条件**：
必须拥有启用了 **triple store** 功能的 SAP HANA Cloud 实例。
详细说明参见：[Enable Triple Store](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide/enable-triple-store/)<br />
加载 `kgdocu_movies` 示例数据。参见 [Knowledge Graph Example](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide/knowledge-graph-example)。

下面将：

1. 实例化一个指向 “movies” 数据图的 `HanaRdfGraph`
2. 用 LLM 驱动的 `HanaSparqlQAChain` 对它进行封装
3. 提出自然语言问题，并打印链的响应

这演示了 LLM 如何在底层生成 SPARQL、在 SAP HANA 上执行查询，并返回可读的答案。

首先，创建到 SAP HANA Cloud 实例的连接。

```python
import os

from dotenv import load_dotenv
from hdbcli import dbapi

# 如果需要则加载环境变量
load_dotenv()

# 建立到 SAP HANA Cloud 的连接
connection = dbapi.connect(
    address=os.environ.get("HANA_DB_ADDRESS"),
    port=os.environ.get("HANA_DB_PORT"),
    user=os.environ.get("HANA_DB_USER"),
    password=os.environ.get("HANA_DB_PASSWORD")
)
```

然后，设置知识图谱实例。

```python
from gen_ai_hub.proxy.langchain.openai import ChatOpenAI
from langchain_hana import HanaRdfGraph, HanaSparqlQAChain

# from langchain_openai import ChatOpenAI  # 或你选择的 LLM
```

```python
# 设置知识图谱
graph_uri = "kgdocu_movies"

graph = HanaRdfGraph(
    connection=connection,
    graph_uri=graph_uri,
    auto_extract_ontology=True
)
```

```python
# 从数据图中提取基础图 schema。这个 schema 会指导 LLM 生成正确的 SPARQL 查询。
schema_graph = graph.get_schema
print(schema_graph.serialize(format="turtle"))
```

```output
@prefix owl: <http://www.w3.org/2002/07/owl#> .
@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> .
@prefix xsd: <http://www.w3.org/2001/XMLSchema#> .

<http://kg.demo.sap.com/acted_in> a owl:ObjectProperty ;
    rdfs:label "acted_in" ;
    rdfs:domain <http://kg.demo.sap.com/Actor> ;
    rdfs:range <http://kg.demo.sap.com/Film> .

<http://kg.demo.sap.com/dateOfBirth> a owl:DatatypeProperty ;
    rdfs:label "dateOfBirth" ;
    rdfs:domain <http://kg.demo.sap.com/Actor> ;
    rdfs:range xsd:dateTime .

<http://kg.demo.sap.com/directed> a owl:ObjectProperty ;
    rdfs:label "directed" ;
    rdfs:domain <http://kg.demo.sap.com/Director> ;
    rdfs:range <http://kg.demo.sap.com/Film> .

<http://kg.demo.sap.com/genre> a owl:ObjectProperty ;
    rdfs:label "genre" ;
    rdfs:domain <http://kg.demo.sap.com/Film> ;
    rdfs:range <http://kg.demo.sap.com/Genre> .

<http://kg.demo.sap.com/placeOfBirth> a owl:ObjectProperty ;
    rdfs:label "placeOfBirth" ;
    rdfs:domain <http://kg.demo.sap.com/Actor> ;
    rdfs:range <http://kg.demo.sap.com/Place> .

<http://kg.demo.sap.com/title> a owl:DatatypeProperty ;
    rdfs:label "title" ;
    rdfs:domain <http://kg.demo.sap.com/Film> ;
    rdfs:range xsd:string .

rdfs:label a owl:DatatypeProperty ;
    rdfs:label "label" ;
    rdfs:domain <http://kg.demo.sap.com/Actor>,
        <http://kg.demo.sap.com/Director>,
        <http://kg.demo.sap.com/Genre>,
        <http://kg.demo.sap.com/Place> ;
    rdfs:range xsd:string .

<http://kg.demo.sap.com/Director> a owl:Class ;
    rdfs:label "Director" .

<http://kg.demo.sap.com/Genre> a owl:Class ;
    rdfs:label "Genre" .

<http://kg.demo.sap.com/Place> a owl:Class ;
    rdfs:label "Place" .

<http://kg.demo.sap.com/Actor> a owl:Class ;
    rdfs:label "Actor" .

<http://kg.demo.sap.com/Film> a owl:Class ;
    rdfs:label "Film" .
```

接着，初始化 LLM。

```python
# 初始化 LLM
llm = ChatOpenAI(proxy_model_name="gpt-4o", temperature=0)
```

然后，创建一个 SPARQL QA Chain。

```python
# 创建 SPARQL QA Chain
chain = HanaSparqlQAChain.from_llm(
    llm=llm,
    verbose=True,
    allow_dangerous_requests=True,
    graph=graph,
)
```

```python
# output = chain.invoke("Which movies are in the data?")
# output = chain.invoke("In which movies did Keanu Reeves and Carrie-Anne Moss play in together")
# output = chain.invoke("which movie genres are in the data?")
# output = chain.invoke("which are the two most assigned movie genres?")
# output = chain.invoke("where were the actors of "Blade Runner" born?")
# output = chain.invoke("which actors acted together in a movie and were born in the same city?")
output = chain.invoke("which actors acted in Blade Runner?")

print(output["result"])
```

```output

