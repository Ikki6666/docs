...
description: "使用 LangChain Python 将聊天会话历史存储到 CockroachDB 中。"
---

`CockroachDBChatMessageHistory` 会将聊天会话历史存储在 CockroachDB 的分布式 SQL 数据库中。

代码位于这个集成包中：[langchain-cockroachdb](https://github.com/cockroachdb/langchain-cockroachdb/)。

## 概览

CockroachDBChatMessageHistory 提供：

- **分布式存储**：聊天历史会自动在节点之间复制
- **强一致性**：SERIALIZABLE 事务可防止消息顺序问题
- **高可用性**：自动故障转移且不会丢失数据
- **会话隔离**：每个会话都有唯一的 session ID
- **PostgreSQL 兼容性**：可从基于 PostgreSQL 的系统轻松迁移
...
<Tip>
**可选**：可通过 `schema` 参数指定 schema 名称（默认值：`"public"`）
</Tip>
