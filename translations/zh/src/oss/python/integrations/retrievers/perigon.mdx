---
title: "Perigon 集成"
description: "使用 LangChain Python 与 Perigon retriever 集成。"
---

import LangchainCommunityUnmaintained from '/snippets/oss/langchain-community-unmaintained.mdx';

Perigon API 套件提供了对全球新闻和事件的快速、结构化访问，帮助你构建实时、数据驱动的产品。无论你是要跟踪新出现的风险、呈现相关文章，还是挖掘关键信息，Perigon 都提供了可编程完成这些任务的工具。

与传统基于关键词的搜索不同，Perigon 的语义搜索能力可以从上下文中理解查询，并返回相关文档。

本笔记本演示如何在 LangChain 中同时使用面向新闻文章和 Wikipedia 内容的 Perigon retriever。
...
要使用此集成，你需要一个 Perigon API key。可以前往 [Perigon.io](https://perigon.io/) 注册获取。
...
ArticlesRetriever 支持利用语义搜索能力检索新闻文章：
...
WikipediaRetriever 提供面向 Wikipedia 内容的语义搜索能力，并带有丰富元数据：
...
你可以根据热度、分类和其他元数据来过滤 Wikipedia 结果：
...
这两个 retriever 都支持异步操作，以获得更好的性能：
...
如需查看 Perigon API 全部特性和配置项的详细文档，请访问 [Perigon API documentation](https://dev.perigon.io/docs)。
