...
如需了解如何设置 SAP HANA vector store，请参阅[文档](/oss/integrations/vectorstores/sap_hanavector)。
...
为了让自查询获得更好的性能，我们会为 HANA 中的 vectorstore 表创建额外的元数据字段：
...
下面进入正题：如何为 HANA vectorstore 构建一个 SelfQueryRetriever：
