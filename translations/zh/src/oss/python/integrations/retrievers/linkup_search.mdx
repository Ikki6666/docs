...
description: "使用 LangChain Python 与 LinkupSearchRetriever retriever 集成。"
---

> [Linkup](https://www.linkup.so/) 提供了一个 API，用来把 LLM 连接到网页和 Linkup Premium Partner sources。

这会帮助你开始使用 LinkupSearchRetriever [retriever](/oss/langchain/retrieval/)。

### 集成详情

| Retriever | Source | Package |
| :--- | :--- | :---: |
`LinkupSearchRetriever` | Web 和 partner sources | langchain-linkup |

## 设置

要使用 Linkup provider，你需要一个有效的 API key。你可以通过 [注册 Linkup](https://app.linkup.so/sign-up) 获取。然后把它设置为 `LINKUP_API_KEY` 环境变量。对于下面的 chain 示例，你还需要把 OpenAI API key 设置为 `OPENAI_API_KEY` 环境变量，你也可以在这里一起设置：

```python
# import os
# os.environ["LINKUP_API_KEY"] = ""  # 填入你的 API key
# os.environ["OPENAI_API_KEY"] = ""  # 填入你的 API key
```

如果你想从单独的查询中获得自动 tracing，也可以取消下面的注释，设置你的 [LangSmith](/langsmith/observability) API key：
...
retriever = LinkupSearchRetriever(
    ...
    linkup_api_key=None,  # API key 可以在这里传入，也可以设置为 LINKUP_API_KEY 环境变量
)
```
