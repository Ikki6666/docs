---
title: "Cohere reranker 集成"
description: "使用 LangChain Python 与 Cohere reranker retriever 集成。"
---

import LangchainCommunityUnmaintained from '/snippets/oss/langchain-community-unmaintained.mdx';

>[Cohere](https://cohere.ai/about) 是一家加拿大初创公司，提供自然语言处理模型，帮助企业改进人与机器的交互。

本笔记本演示如何在 retriever 中使用 [Cohere 的 rerank endpoint](https://docs.cohere.com/docs/reranking)。
...
## 使用 CohereRerank 进行重排

现在用 `ContextualCompressionRetriever` 包装基础 retriever。我们会加入一个 `CohereRerank`，它使用 Cohere rerank endpoint 对返回结果重新排序。  
