---
title: "Cohere RAG 集成"
description: "使用 LangChain Python 与 Cohere RAG retriever 集成。"
---

>[Cohere](https://cohere.ai/about) 是一家加拿大创业公司，提供自然语言处理模型，帮助企业改善人机交互。

这份 notebook 介绍如何开始使用 `Cohere RAG` retriever。它允许你利用各种 connector 来搜索文档，或者直接传入你自己的文档。
...
请注意，connectors 和 documents 不能同时使用。如果你在 `invoke` 方法中提供了 documents，它们会优先生效，而 connectors 在该次请求中不会被使用，上面的示例就展示了这一点。
