...
你需要获取 AI21 API key，并设置 `AI21_API_KEY` 环境变量：
...
本示例展示如何使用 AI21SemanticTextSplitter 按语义将文本拆分为多个块。
...
本示例展示如何使用 AI21SemanticTextSplitter 按语义拆分文本，然后根据 `chunk_size` 合并拆分后的文本块。
...
本示例展示如何使用 AI21SemanticTextSplitter 按语义将文本拆分为 Documents。每个文档的元数据中会包含一个类型字段。
...
本示例展示如何使用 AI21SemanticTextSplitter 从文本创建 Documents，并为每个 Document 添加自定义元数据。
...
本示例展示如何使用 AI21SemanticTextSplitter 按语义将文本拆分为 Documents。每个文档的元数据中会包含起始索引。
**注意**，这里的起始索引表示块的顺序提示，而不是每个块的真实起始位置。
...
本示例展示如何使用 AI21SemanticTextSplitter 按语义将 Documents 列表拆分为多个块。
