---
title: "Volcengine reranker 集成"
description: "使用 LangChain Python 与 Volcengine reranker 文档转换器集成。"
---

import LangchainCommunityUnmaintained from '/snippets/oss/langchain-community-unmaintained.mdx';

本 notebook 展示如何使用 Volcengine Reranker 进行文档压缩和检索。[Volcengine](https://www.volcengine.com/) 是字节跳动（TikTok 的母公司）开发的云服务平台。

Volcengine 的 Rerank Service 支持最多对 50 个文档重新排序，最大输入长度为 4000 tokens。更多信息请参见 [the Volcengine Rerank API documentation](https://www.volcengine.com/docs/84313/1254474) 和 [the Rerank service guide](https://www.volcengine.com/docs/84313/1254605)。
...
我们先初始化一个简单的向量存储检索器，并将 2023 年国情咨文演讲（分块后）存入其中。这里可以将检索器设置为返回较多文档（20 个）。
...
## 使用 VolcengineRerank 重新排序

现在我们用 `ContextualCompressionRetriever` 包装基础检索器，并使用 `VolcengineRerank` 对返回结果重新排序。
