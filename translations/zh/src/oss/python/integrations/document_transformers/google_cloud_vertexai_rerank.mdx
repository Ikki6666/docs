---
title: "Google cloud Vertex AI reranker 集成"
description: "使用 LangChain Python 与 Google cloud Vertex AI reranker 文档转换器集成。"
---

import LangchainCommunityUnmaintained from '/snippets/oss/langchain-community-unmaintained.mdx';

> [Vertex Search Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking) 是 [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis) 中的独立 API 之一。它接收一组文档，并根据这些文档与查询的相关性对其重新排序。与只关注文档和查询语义相似度的 embeddings 相比，ranking API 能为文档回答特定查询的效果提供更精确的分数。你可以在检索到初始候选文档后使用 ranking API 提升搜索结果质量。

>The ranking API 是无状态的，因此在调用前无需为文档建立索引。你只需要传入查询和文档。这使得它非常适合对任何文档检索器返回的文档进行 rerank。

>更多信息请参见 [Rank and rerank documents](https://cloud.google.com/generative-ai-app-builder/docs/ranking)。
...
在这个示例中，我们将使用 [Google Wiki page](https://en.wikipedia.org/wiki/Google) 来演示 Vertex Ranking API 的工作方式。

这里使用标准流程 `load -> split -> embed data`。

embeddings 使用 [Vertex Embeddings API](https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-text-embeddings#supported_models) 中的 `textembedding-gecko@003` 模型创建。
...
让我们使用同一个查询同时调用 `basic_retriever` 和 `retriever_with_reranker`，并比较它们检索到的文档。

Ranking API 会接收 `basic_retriever` 的输入，并将其传递给 Ranking API。

ranking API 用于提升排序质量，并给出一个表示每条记录与查询相关性的分数。

你可以看到未重排文档与已重排文档之间的差异。Ranking API 会把语义上最相关的文档移到 LLM 上下文窗口的顶部，从而帮助模型给出推理更充分的回答。
...
让我们检查几条重排后的文档。可以看到，检索器仍然返回相关的 LangChain [documents](https://reference.langchain.com/python/langchain-core/documents/base/Document) 类型，同时在 `metadata` 字段中还会包含来自 Ranking API 的 `relevance_score`。
...
下面展示一个完整的 RAG chain 示例，其中使用了一个简单的 prompt template 来演示如何通过 Vertex Ranking API 执行 reranking。
