---
title: "Google translate 集成"
description: "使用 LangChain Python 与 Google translate 文档转换器集成。"
---

[Google Translate](https://translate.google.com/) 是 Google 开发的多语言神经机器翻译服务，可将文本、文档和网站从一种语言翻译成另一种语言。

`GoogleTranslateTransformer` 允许你通过 [Google Cloud Translation API](https://cloud.google.com/translate) 翻译文本和 HTML。

要使用它，你需要安装 `google-cloud-translate` Python 包，并准备一个已启用 [Translation API](https://cloud.google.com/translate/docs/setup) 的 Google Cloud 项目。该转换器使用的是 [Advanced edition (v3)](https://cloud.google.com/translate/docs/intro-to-v3)。
...
初始化 `GoogleTranslateTransformer` 时，你可以包含以下参数来配置请求。

- `project_id`: Google Cloud Project ID。
- `location`: （可选）Translate model location。
  - 默认值：`global`
- `model_id`: （可选）要使用的 Translate [model ID][models]。
- `glossary_id`: （可选）要使用的 Translate [glossary ID][glossaries]。
- `api_endpoint`: （可选）要使用的 [Regional endpoint][endpoints]。
...
翻译文档后，结果会作为一个新的 document 返回，其中 `page_content` 已被翻译为目标语言。

你可以向 `transform_documents()` 方法传入以下关键字参数：

- `target_language_code`: 输出文档的 [ISO 639][iso-639] 语言代码。
  - 支持的语言参见 [Language support][supported-languages]。
- `source_language_code`: （可选）输入文档的 [ISO 639][iso-639] 语言代码。
  - 如果不提供，则会自动检测语言。
- `mime_type`: （可选）输入文本的 [Media Type][media-type]。
  - 可选值：`text/plain`（默认）、`text/html`。
