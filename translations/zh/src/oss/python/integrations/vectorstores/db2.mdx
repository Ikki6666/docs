...
LangChain 的 Db2 集成（`langchain-db2`）为 IBM 关系型数据库 Db2 v12.1.2 及以上版本提供向量存储与向量搜索能力，基于 MIT 许可证发布。用户既可以直接使用其提供的实现，也可以根据具体需求进行定制。其主要特性包括：

* 带元数据的向量存储
* 向量相似度搜索与最大边际相关性搜索，并支持元数据过滤
* 支持点积、余弦和欧几里得距离度量
* 通过索引创建和近似最近邻搜索来提升性能（即将加入）

## 设置

### 在 LangChain 中使用 Db2 向量存储与搜索的前置条件

安装 `langchain-db2` 包，它是 db2 LangChain Vector Store and Search 的集成包。

安装该包时，也应该会一并安装 `langchain-core` 和 `ibm_db` 等依赖。
...
下面的示例代码演示了如何连接到 Db2 Database。除了上面的依赖之外，你还需要一个正在运行的 Db2 数据库实例（版本为 v12.1.2+，支持 vector datatype）。
...
首先我们会创建三个向量存储，每个使用不同的距离策略。

（你可以手动连接到 Db2 Database，然后会看到三个表：`Documents_DOT`、`Documents_COSINE` 和 `Documents_EUCLIDEAN`。）
...
使用过滤时，我们只选择文档 id 为 101 的文档，不选择其他文档。
