---
title: "Kinetica 向量存储集成"
description: "使用 LangChain Python 集成 Kinetica VectorStore API 向量存储。"
---
...
本笔记本介绍如何使用 Kinetica 向量存储（`Kinetica`）。

你需要先准备一个 Kinetica 实例，可以按照这里的说明快速搭建：[installation instruction](https://www.kinetica.com/developer-edition/)。
...
我们将使用 `OpenAIEmbeddings`，因此需要先获取 OpenAI API Key。
...
你必须通过以下环境变量设置数据库连接。如果你使用虚拟环境，也可以在项目的 `.env` 文件中配置：
...
Kinetica 模块会尝试使用 collection 名称创建数据表。
请确保 collection 名称唯一，并且当前用户有创建表的权限。
...
上面我们从零开始创建了一个向量存储。不过在很多场景里，你会希望直接操作一个已经存在的向量存储。
这种情况下，可以像下面这样直接初始化它。
...
如果你已经有一个现有 collection，可以通过 `from_documents` 并设置 `pre_delete_collection=True` 来覆盖它。
