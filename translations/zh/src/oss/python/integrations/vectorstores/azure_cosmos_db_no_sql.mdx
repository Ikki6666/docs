---
title: "Azure Cosmos DB NoSQL 集成"
description: "使用 LangChain Python 集成 Azure Cosmos DB NoSQL 向量存储。"
---

import LangchainCommunityUnmaintained from '/snippets/oss/langchain-community-unmaintained.mdx';

本笔记本展示了如何利用这个集成的[向量数据库](https://learn.microsoft.com/en-us/azure/cosmos-db/vector-database)，在集合中存储文档、创建索引，并使用 COS（余弦距离）、L2（欧几里得距离）和 IP（内积）等近似最近邻算法执行向量搜索查询，以定位与查询向量接近的文档。

Azure Cosmos DB 是为 OpenAI 的 ChatGPT 服务提供支持的数据库。它提供个位数毫秒级响应时间、自动且即时的可扩展性，以及任意规模下的速度保证。

Azure Cosmos DB for NoSQL 现已在预览版中提供向量索引和搜索功能。该能力专为处理高维向量而设计，可在任意规模下实现高效、准确的向量搜索。你现在可以将向量与数据一起直接存储在文档中。这意味着数据库中的每个文档不仅可以包含传统的无模式数据，也可以把高维向量作为其他属性一并保存。数据与向量的共置使索引和搜索更高效，因为向量与其所表示的数据存储在同一逻辑单元中。这可以简化数据管理、AI 应用架构，以及基于向量的操作效率。
...
[立即注册](https://azure.microsoft.com/en-us/free/) 以获得永久免费访问并开始使用。

<Note>
从 `langchain-azure-cosmosdb` v1.0 开始，所有 Azure Cosmos DB NoSQL 集成（向量存储、语义缓存、聊天消息历史，以及 LangGraph checkpointer、cache 和 store）都通过独立的 [`langchain-azure-cosmosdb`](https://pypi.org/project/langchain-azure-cosmosdb/) 包发布，并从 `langchain_azure_cosmosdb` 导入。同时支持同步与异步客户端，以及 Microsoft Entra ID（`DefaultAzureCredential`）认证。
</Note>
