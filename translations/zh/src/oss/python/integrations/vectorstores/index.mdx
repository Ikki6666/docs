...
title: "向量存储集成"
sidebarTitle: "向量存储"
description: "使用 LangChain Python 集成向量存储。"
---
...
向量存储会保存[嵌入](/oss/integrations/embeddings)后的数据，并执行相似度搜索。
...
LangChain 为向量存储提供了统一接口，让你可以：

- `add_documents` - 向存储中添加文档。
- `delete` - 按 ID 删除已存储的文档。
- `similarity_search` - 查询语义上相似的文档。

这种抽象让你无需修改应用逻辑，就能在不同实现之间切换。

### 初始化

要初始化向量存储，请为其提供一个嵌入模型：
...
像下面这样添加 @[`Document`] 对象（包含 `page_content` 和可选元数据）：
...
使用 `similarity_search` 发起语义查询，它会返回嵌入后最接近的文档：
...
高效搜索通常会使用诸如 HNSW（Hierarchical Navigable Small World）之类的索引方法，不过具体实现取决于所选向量存储。

### 元数据过滤

按元数据过滤（例如来源、日期）可以帮助你进一步优化搜索结果：
...
更多信息，请参见 [Voyage AI 文档](https://www.mongodb.com/docs/voyageai/models/text-embeddings/)。
</Accordion>
...
使用 [start-local](https://github.com/elastic/start-local) 脚本安装软件包并在本地启动 Elasticsearch：
...
Elasticsearch 将在 `http://localhost:9200` 上可用。`elastic` 用户的密码和 API key 存储在 `elastic-start-local` 文件夹中的 `.env` 文件里。
...
更多信息，请参见 [MongoDB LangChain 集成文档](https://www.mongodb.com/docs/atlas/ai-integrations/langchain/#vector-store)。
</Accordion>
...
# 假设本地 Weaviate 实例运行在 http://localhost:8080，gRPC 端口为 50051。
# 其他部署方式（Weaviate Cloud、Docker 等）请参见 Weaviate 指南。
