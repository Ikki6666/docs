---
title: "Docugami 集成"
description: "使用 LangChain Python 集成 Docugami 文档加载器。"
---

本文介绍如何从 `Docugami` 加载文档，以及使用该系统相较其他数据加载器的优势。

## 先决条件

1. 安装所需的 Python 包。
2. 为你的 workspace 获取 access token，并确保将其设置为 `DOCUGAMI_API_KEY` 环境变量。
3. 按照这里的说明，为已处理的文档获取一些 docset 和 document ID：[help.docugami.com/home/docugami-api](https://help.docugami.com/home/docugami-api)
...
1. 创建一个 [Docugami workspace](http://www.docugami.com)（提供免费试用）。
2. 添加你的文档（PDF、DOCX 或 DOC），并让 Docugami 对其进行摄取和聚类，将相似文档分到同一组中，例如 NDA、租赁协议和服务协议。系统并没有预设固定支持的文档类型，最终形成的聚类取决于你的实际文档，而且你之后也可以[修改 docset assignment](https://help.docugami.com/home/working-with-the-doc-sets-view)。
3. 通过你 workspace 的 Developer Playground 创建 access token。详见 [Detailed instructions](https://help.docugami.com/home/docugami-api)。
4. 浏览 [Docugami API](https://api-docs.docugami.com)，拿到已处理 docset 的 ID 列表，或者某个特定 docset 下的 document ID。
6. 按照下文说明使用 DocugamiLoader，为你的文档获取富语义切块。
7. （可选）构建并发布一个或多个 [reports or abstracts](https://help.docugami.com/home/reports)。这能帮助 Docugami 按照你的偏好改进语义 XML，并将更好的标签作为 metadata 加入 DocugamiLoader 输出。你还可以配合 self-querying retriever 等技术，实现高精度的 Document QA。

## 相比其他切块技术的优势

对文档进行恰当切块，对后续检索至关重要。常见的切块方法很多，包括依赖空白字符的简单做法，以及基于字符长度递归拆分的方法。Docugami 提供了不同的方式：

1. **智能切块：** Docugami 会把每份文档拆解成层级化的语义 XML 树，包含不同大小的块，从单个词或数值一直到完整章节。这些块遵循文档自身的语义轮廓，比任意长度切块或基于空白字符的简单切块更有意义。
2. **语义标注：** 这些块会带有跨整个文档集保持一致的语义标签，因此即使多份文档的写法和排版不同，也能在它们之间执行一致的层级查询。例如，在一组租赁协议中，你可以轻松定位房东、租户、续约日期等关键信息，也能检索更复杂的内容，例如转租条款的具体措辞，或者某个司法辖区是否在终止条款中包含例外章节。
3. **结构化表示：** 此外，XML 树还会描述每份文档的结构轮廓，通过属性标识标题、段落、列表、表格及其他常见元素，并且这种表示方式在扫描版 PDF、DOCX 等所有支持的文档格式中都保持一致。它也能正确处理长文档中的页眉页脚、多栏排版等特性，从而得到更干净的文本提取结果。
4. **额外 metadata：** 如果用户一直在使用 Docugami，块还会带有额外的 metadata。这些 metadata 可以帮助你在不受上下文窗口限制的情况下实现高精度的 Document QA。下文会给出更详细的代码 walkthrough。
...
如果已经设置了 `DOCUGAMI_API_KEY` 环境变量，就不需要显式传给加载器。否则，你也可以通过 `access_token` 参数传入。
...
每个 @[`Document`] 的 `metadata`（实际上是 PDF、DOC 或 DOCX 中某个 chunk 的 metadata）包含一些额外有用的信息：

1. **id and source：** 该 chunk 在 Docugami 中来源文件（PDF、DOC 或 DOCX）的 ID 和名称。
2. **xpath：** 该 chunk 在文档 XML 表示中的 XPath。它有助于把引用直接定位回文档 XML 中的实际 chunk。
3. **structure：** chunk 的结构属性，例如 h1、h2、div、table、td 等。调用方如有需要，可以利用它过滤某些类型的 chunk。
4. **tag：** chunk 的语义标签，基于多种生成式与抽取式技术生成。详见：[github.com/docugami/DFM-benchmarks](https://github.com/docugami/DFM-benchmarks)

你可以通过设置 `DocugamiLoader` 实例上的以下属性来控制切块行为：

1. 你可以设置最小和最大 chunk 大小，系统会尽量在最少截断的前提下遵循这些限制。可以通过 `loader.min_text_length` 和 `loader.max_text_length` 控制。
2. 默认情况下，返回的只有 chunk 文本。但 Docugami 的 XML knowledge graph 中还包含其他丰富信息，包括 chunk 内部实体的语义标签。如果你需要在返回的 chunk 中附带额外 XML metadata，可以设置 `loader.include_xml_tags = True`。
3. 此外，如果你希望 Docugami 在返回 chunk 时一并返回父级 chunk，可以设置 `loader.parent_hierarchy_levels`。子 chunk 会通过 `loader.parent_id_key` 指向父 chunk。这在配合 MultiVector Retriever 做 [small-to-big](https://www.youtube.com/watch?v=ihSiRrOUwmg) 检索时尤其有用。本文后面会给出更详细的示例。
...
## 基础用法：使用 Docugami loader 做文档问答

你可以像使用标准 loader 一样使用 Docugami Loader，对多份文档执行 Document QA，只是它的切块质量更高，也更贴合文档自然结构。关于这类用法已经有许多不错的教程，例如 [this one](https://www.youtube.com/watch?v=3yPBVii7Ct0)。你可以直接沿用同样的代码，只是把基础文本或 PDF 加载和切分方式替换为 `DocugamiLoader`，从而获得更好的切块。
...
loader 返回的文档本身已经切好块，因此不需要再使用文本切分器。你也可以按需利用每个文档上的 metadata，例如 `structure` 或 `tag` 属性，执行后处理。

这里我们直接按常规方式，使用 `DocugamiLoader` 的输出构建 retrieval QA chain。
...
## 使用 docugami knowledge graph 实现高精度文档问答

大型文档的一个难点在于，正确答案往往依赖于文档中相距很远的多个 chunk。典型的切块技术即使带 overlap，也很难为 LLM 提供足够上下文来回答这类问题。未来即便有超大上下文窗口的 LLM，也只是延后这个问题，面对极长文档或大量文档时仍会遇到限制。

例如，如果我们提出一个更复杂的问题，需要 LLM 同时利用文档不同位置的 chunk，即使使用 OpenAI 的强大模型，也可能无法正确回答。
...
乍看之下，这个回答似乎合理，但其实是错误的。如果你仔细检查这次回答的 source chunk，会发现文档切块并没有把房东名称和可出租面积放进同一段上下文里，还检索出了无关 chunk，因此答案错误（正确答案应为 **13,500 sq ft**）。

Docugami 可以解决这一点。如果用户一直在 [使用 Docugami](https://help.docugami.com/home/reports)，系统会用多种技术为 chunk 附加额外 metadata。后续还会加入更多技术方案。

具体来说，我们可以让 Docugami 在输出中返回 XML tags 以及额外 metadata：
...
借助这些额外 metadata，我们可以使用 self-querying retriever 来提升查询精度：
...
再次运行同样的问题。由于 self-querying retriever 会根据 metadata 中的 landlord 属性构造过滤条件，并且只保留与 DHA Group 房东对应的文档，因此这次能够返回正确结果。即使包含正确答案的具体 chunk 本身没有直接提到 landlord，这样做依然提高了回答准确率。
