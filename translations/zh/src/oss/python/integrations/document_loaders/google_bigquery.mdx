...
>[Google BigQuery](https://cloud.google.com/bigquery) 是一个无服务器、具备成本效益的企业级数据仓库，可跨云运行并随数据规模扩展。
`BigQuery` 是 `Google Cloud Platform` 的一部分。

将一个 `BigQuery` 查询加载为文档，每行对应一个 document。
