...
> 可以从多种文件格式中提取文档元素（文本、图片、复杂表格、图表等），
> 并将其转换为结构化 JSON，便于集成进 RAG 系统。
...
- `POLARIS_AI_DATA_INSIGHT_API_KEY`：你的 Polaris AI DataInsight API key。请阅读 [Polaris AI DataInsight Documentation](https://datainsight.polarisoffice.com/api/keys) 获取 API key。
...
随后，你会看到类似下面这样的文档提取内容与 metadata：
