---
title: "ChatCohere 集成"
description: "使用 LangChain Python 集成 Cohere 聊天模型。"
---

本笔记本介绍如何开始使用 [Cohere chat models](https://cohere.com/chat)。

如需查看全部属性和方法的详细文档，请前往 [API reference](https://reference.langchain.com/python/langchain-cohere/chat_models/ChatCohere)。

## 设置

该集成位于 `langchain-cohere` 包中，可以通过以下命令安装：

```bash
pip install -U langchain-cohere
```

还需要获取一个 [Cohere API key](https://cohere.com/)，并设置 `COHERE_API_KEY` 环境变量：
...
同时也很建议设置 [LangSmith](https://smith.langchain.com)，以获得一流的可观测性，不过这不是必需的。
...
ChatCohere 支持全部 [ChatModel](/oss/langchain/models) 能力：
