...
本页将帮助你开始使用 Qwen [聊天模型](/oss/langchain/models)。如需查看 `ChatQwen` 全部功能和配置项的详细文档，请前往 [API reference](https://pypi.org/project/langchain-qwq/)。
...
要访问 Qwen 模型，你需要创建一个阿里云账户、获取 API key，并安装 `langchain-qwq` 集成包。
...
LangChain 的 Qwen 集成位于 `langchain-qwq` 包中：
...
ChatQwen 支持 tool calling API。你可以描述工具及其参数，并让模型返回一个 JSON 对象，其中包含要调用的工具以及该工具的输入。
...
如需查看 `ChatQwen` 全部功能和配置项的详细文档，请前往 [API reference](https://pypi.org/project/langchain-qwq/)
