...
本指南简要介绍如何开始使用 Cerebras [chat models](/oss/langchain/models)。如需查看 `ChatCerebras` 全部功能和配置的详细文档，请前往 [API reference](https://reference.langchain.com/python/langchain-cerebras/chat_models/ChatCerebras)。

在 Cerebras，我们开发了全球最大、速度最快的 AI 处理器，Wafer-Scale Engine-3（WSE-3）。由 WSE-3 驱动的 Cerebras CS-3 系统代表了一类全新的 AI 超级计算机，在生成式 AI 训练和推理方面，以无与伦比的性能和可扩展性树立了新标准。

选择 Cerebras 作为推理提供方后，你可以：

- 为 AI 推理工作负载获得前所未有的速度
- 以高吞吐量进行商业化构建
- 借助无缝集群技术轻松扩展 AI 工作负载

我们的 CS-3 系统可以快速、轻松地进行集群化，构建全球最大的 AI 超级计算机，让部署和运行超大模型变得简单。领先企业、研究机构和政府已经在使用 Cerebras 方案来开发专有模型，并训练流行的开源模型。

想体验 Cerebras 的能力？可以访问我们的 [website](https://cerebras.ai) 获取更多资源，并探索如何通过 Cerebras Cloud 或本地部署来访问我们的技术。

如需了解 Cerebras Cloud 的更多信息，请访问 [cloud.cerebras.ai](https://cloud.cerebras.ai/)。API 参考文档位于 [inference-docs.cerebras.ai](https://inference-docs.cerebras.ai/)。
...
从 [cloud.cerebras.ai](https://cloud.cerebras.ai/) 获取 API key，并将其加入环境变量：
...
如需查看 `ChatCerebras` 全部功能和配置的详细文档，请前往 [API reference](https://reference.langchain.com/python/langchain-cerebras/chat_models/ChatCerebras)
