...
服务层级（service tier）决定了请求计费所对应的价格与性能档位。使用 `flex` 可以为对延迟不敏感的任务支付更低费用，使用 `priority` 则可以付出溢价换取更低延迟。省略该参数时，请求会使用 standard 层级。

<Note>
    `service_tier` 要求 `langchain-google-genai>=4.4.0`。
</Note>

可以在模型上设置该层级，也可以按请求传入：
...
可接受的取值为 `"standard"`、`"flex"` 和 `"priority"`。该取值在两个后端上含义相同，集成会按照各后端期望的方式发送：在 Gemini Developer API 上作为请求体字段，在 Vertex AI 上作为请求头。

某个层级必须同时针对项目和模型启用。不可用的层级会返回 `400`，而不会回退到 standard 层级，因此请求会直接失败，而不是以你并未请求的价格计费：

```python
ClientError: 400 INVALID_ARGUMENT. Flex API is not supported for model: gemini-2.5-flash.
```

更多信息请参阅 Gemini Developer API 关于 [Flex](https://ai.google.dev/gemini-api/docs/flex-inference) 和 [Priority](https://ai.google.dev/gemini-api/docs/priority-inference) 的指南，以及 Vertex AI 关于 [Flex](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/flex-paygo) 和 [Priority](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/priority-paygo) 的指南。
