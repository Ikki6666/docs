...
> [Seekr](https://www.seekr.com/) 提供由 AI 驱动的解决方案，用于实现结构化、可解释且透明的 AI 交互。

本指南简要介绍如何开始使用 `ChatSeekrFlow` [聊天模型](/oss/langchain/models)。

## 概览

`ChatSeekrFlow` 类封装了部署在 SeekrFlow 上的聊天模型 endpoint，可与 LangChain 应用无缝集成。
...
`ChatSeekrFlow` 所封装的服务 endpoint **必须** 使用 OpenAI-compatible 的聊天输入/输出格式。它可用于：

1. **Fine-tuned Seekr models**
2. **Custom SeekrFlow models**
3. **RAG-enabled models using Seekr's retrieval system**

如果需要异步用法，请参考 `AsyncChatSeekrFlow`（即将推出）。

# 在 LangChain 中开始使用 ChatSeekrFlow

本 notebook 介绍如何在 LangChain 中把 SeekrFlow 当作聊天模型使用。
