...
<Note>
    `reasoning_effort` 需要 `langchain-aws>=1.6.5`。
</Note>

[`ChatBedrockConverse`](https://reference.langchain.com/python/langchain-aws/chat_models/bedrock_converse/ChatBedrockConverse) 支持标准的 [`reasoning_effort`](/oss/langchain/models#reasoning) 参数，适用于具备可配置推理能力的模型，包括 Bedrock 上的 Claude（Opus 5、Sonnet 5）、Amazon Nova 2，以及 OpenAI gpt-oss 和 Moonshot Kimi K2 模型。与 `temperature` 类似，它可以在构造模型时设置，也可以在每次调用时设置，并会被转换为底层模型家族对应的请求格式：
...
支持的力度级别因模型而异。请查看模型的 [profile](/oss/langchain/models#model-profiles) 了解其支持的级别：

```python
llm.profile["reasoning_effort_levels"]  # e.g. ['low', 'medium', 'high', 'xhigh', 'max']
```

显式指定的 `additional_model_request_fields` 值优先于由 `reasoning_effort` 推导出的字段。没有已知转换方式的模型会忽略 `reasoning_effort` 并发出警告。
