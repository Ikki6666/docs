...
description: "使用 LangChain Python 集成 ChatGradient 聊天模型。"
---

本页帮助你开始在 DigitalOcean Gradient [chat models](/oss/langchain/models) 中使用 `ChatGradient`。
...
`langchain-gradient` 使用 DigitalOcean Gradient Platform。

请创建一个 DigitalOcean 账号，从 Gradient Platform 获取 `DIGITALOCEAN_INFERENCE_KEY` API key，并安装 `langchain-gradient` 集成包。

### 凭证

前往 [DigitalOcean Login](https://cloud.digitalocean.com/login)

1. 注册或登录 DigitalOcean Cloud Console
2. 进入 Gradient Platform，并导航到 Serverless Inference。
3. 点击 Create model access key，输入名称，然后创建 key。

完成后，设置 `DIGITALOCEAN_INFERENCE_KEY` 环境变量：
...
可以像下面这样将模型与 prompt template 组合起来：
...
如需查看 `ChatGradient` 全部功能和配置的详细文档，请前往 API reference。
