...
description: 使用 LangChain 中的旧版 Gemini LLM。
---

<Warning>
**你当前所在的页面文档介绍的是 Google 模型作为文本补全模型。许多常用的 Google 模型其实是 [聊天补全模型](/oss/langchain/models)。**

你可能更想查看 [这个页面](/oss/integrations/chat/google_generative_ai/)。
</Warning>

本页介绍如何在 LangChain 中使用 [Google Generative AI](https://developers.generativeai.google/) 模型。注意：它与 Google Cloud Vertex AI [集成](/oss/integrations/llms/google_vertex_ai) 是分开的。

## 配置

要使用 Google Generative AI，你必须安装 `langchain-google-genai` Python 包并生成一个 API key。[查看详细信息](https://developers.generativeai.google/)。
...
Gemini 模型有默认安全设置，你可以覆盖它们。如果你的模型经常收到大量 “Safety Warnings”，可以尝试调整模型的 `safety_settings` 属性。例如，要关闭对危险内容的安全阻断，你可以这样构造 LLM：
...
关于可用类别和阈值的列表，请参见 Google 的 [安全设置指南](https://ai.google.dev/gemini-api/docs/safety-settings)。
