...
ModelScope ([Home](https://www.modelscope.cn/) | [GitHub](https://github.com/modelscope/modelscope)) 基于 “Model-as-a-Service” (MaaS) 理念构建。它希望汇聚 AI 社区中最先进的机器学习模型，并简化在真实应用中利用 AI 模型的流程。这个仓库中开源的核心 ModelScope 库提供了接口和实现，让开发者能够执行模型推理、训练和评估。这将帮助你开始使用 LangChain 来接入 ModelScope completion 模型（LLMs）。
...
要访问 ModelScope 模型，你需要创建一个 ModelScope 账号、获取 SDK token，并安装 `langchain-modelscope-integration` 集成包。

### 凭据

前往 [ModelScope](https://modelscope.cn/) 注册并生成一个 [SDK token](https://modelscope.cn/my/myaccesstoken)。完成后，设置 `MODELSCOPE_SDK_TOKEN` 环境变量：
...
更多细节请参见 [modelscope.cn/docs/model-service/API-Inference/intro](https://modelscope.cn/docs/model-service/API-Inference/intro)。
