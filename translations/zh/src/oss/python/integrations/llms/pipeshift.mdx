---
title: "Pipeshift 集成"
description: "使用 LangChain Python 集成 Pipeshift LLM。"
---

本文将帮助你开始使用 LangChain 配合 Pipeshift completion models (LLMs)。有关 `Pipeshift` 功能和配置选项的详细文档，请参阅 [API reference](https://dashboard.pipeshift.com/docs)。
...
要访问 Pipeshift 模型，你需要创建一个 Pipeshift 账户、获取 API key，并安装 `langchain-pipeshift` 集成包。
...
如果你想为模型调用启用自动 tracing，也可以取消下面的注释来设置你的 [LangSmith](/langsmith/observability) API key：
...
有关 `Pipeshift` 所有功能和配置的详细文档，请参阅 API reference：[dashboard.pipeshift.com/docs](https://dashboard.pipeshift.com/docs)
