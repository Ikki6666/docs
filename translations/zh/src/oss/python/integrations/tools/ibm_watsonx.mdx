...
>`WatsonxToolkit` 是 IBM [watsonx.ai](https://www.ibm.com/products/watsonx-ai) Toolkit 的包装器。

本示例展示了如何通过 `LangChain` 使用 `watsonx.ai` Toolkit。
...
要访问 IBM watsonx.ai toolkit，你需要创建 IBM watsonx.ai 账户、获取 API key，并安装 `langchain-ibm` 集成包。

### 凭证

这个单元定义了使用 watsonx Toolkit 所需的 WML 凭证。

**操作：** 提供 IBM Cloud 用户 API key。详情请参阅
...
或者，你也可以使用 Cloud Pak for Data 凭证。详情请参阅 [watsonx.ai software setup](https://ibm.github.io/watsonx-ai-python-sdk/setup_cpd.html)。

在某些场景下，还可以将 IBM 的 [`APIClient`](https://ibm.github.io/watsonx-ai-python-sdk/base.html#apiclient) 对象传入 `WatsonxToolkit` 类。
...
你可以将所有可用工具作为 `WatsonxTool` 对象列表获取。
...
<Note>
    可用工具列表可能会因为你使用的是 IBM Cloud 版 IBM watsonx.ai，还是 IBM watsonx.ai software 而有所不同。
</Note>
...
如果你想检查某个工具是否有 config schema，并查看它的属性，可以查看该工具的 `tool_config_schema`。

在这个示例中，该工具有一个 config schema，其中包含 `maxResults` 参数，可用于设置最大返回结果数。
...
要设置 `tool_config` 参数，你需要使用 `set_tool_config()` 方法，并根据上面的 `tool_config_schema` 传入正确的 `dict`。
...
为了演示这一点，我们需要再获取一个带有输入 schema 的工具。

```python
weather_tool = watsonx_toolkit.get_tool("Weather")
```

如果你想检查某个工具是否有输入 schema，并查看它的属性，可以查看该工具的 `tool_input_schema`。

在这个示例中，该工具有一个输入 schema，其中包含一个必填参数和一个可选参数。
...
要正确地把输入传给 `invoke()`，你需要创建一个 `invoke_input` 字典，将必填参数作为 key，并填入对应值。
...
我们也可以使用 ToolCall 调用该工具，这种情况下会返回一个 ToolMessage：
...
如需了解 `WatsonxToolkit` 全部特性与配置的详细文档，请参阅 [API reference](https://reference.langchain.com/python/integrations/langchain_ibm/WatsonxToolkit/)。
