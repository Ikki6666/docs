...
本笔记本概述了如何借助 [langchain-bodo](https://pypi.org/project/langchain-bodo/) 集成包创建 agents，并在大型数据集上执行问答，该包底层使用 [Bodo DataFrames](https://github.com/bodo-ai/Bodo) 和 `Python` agent。

Bodo DataFrames 是一个高性能 DataFrame 库，只需简单修改导入方式，就可以自动加速并扩展 Pandas 代码（见下方示例）。由于它与 Pandas 具有很强的兼容性，Bodo DataFrames 使得通常擅长生成 Pandas 代码的 LLM 能够更高效地回答关于更大数据集的问题，并让生成的代码突破 Pandas 的限制。

**NOTE: `Python` agent 会执行 LLM 生成的 Python 代码，如果这些代码有害，就会带来风险。请谨慎使用。**

## 设置

运行示例之前，请复制 [titanic dataset](https://raw.githubusercontent.com/pandas-dev/pandas/main/doc/data/titanic.csv)，并在本地保存为 `titanic.csv`。

安装 `langchain-bodo` 时，也会同时安装 Bodo 和 Pandas 依赖：
...
Bodo DataFrames 免费使用，不需要额外凭证。
示例中使用 OpenAI 模型，如果你尚未配置，请设置 `OPENAI_API_KEY`：
...
以下示例借鉴自 [Pandas DataFrames agent notebook](/oss/integrations/tools/pandas)，并做了一些修改，以突出关键差异。

第一个示例展示了如何将 Bodo DataFrame 直接传给 `create_bodo_dataframes_agent`，并提出一个简单问题。
...
这展示了如何使用 `ZERO_SHOT_REACT_DESCRIPTION` agent type 初始化 agent。
...
这展示了如何使用 `OPENAI_FUNCTIONS` agent type 初始化 agent。注意，这是上面方式的另一种选择。
...
## 结合 bodo DataFrames 和预处理来创建并调用 agents

这个示例展示了一个稍复杂的用例：把带有附加预处理的 Bodo DataFrame 传给 `create_bodo_dataframes_agent`。
由于 Bodo DataFrames 是惰性求值的，如果回答问题时不需要所有列，你就有机会节省计算量。注意，传给 agent 的 DataFrame 也可以大于可用内存。
...
你也可以向 agent 传入多个 DataFrame。
需要注意的是，虽然 Bodo DataFrames 支持 Pandas 中大多数常见的高计算量操作，但如果 agent 生成了当前尚不支持的代码（见下方警告），这些 DataFrame 会回退转换为 Pandas，以避免报错。

如需查看当前已支持特性的更多细节，请参阅 [Bodo DataFrames API documentation](https://docs.bodo.ai/latest/api_docs/dataframe_lib/)。
...
## 使用 `number_of_head_rows` 优化 agent 调用

默认情况下，DataFrame 的头部行会以 markdown 表格形式嵌入到 prompt 中。
由于 Bodo DataFrames 是惰性求值的，这个 head 操作可以被优化，但在某些情况下仍然会比较慢。作为一种优化方式，你可以将 head 的行数设为 0，这样在构造 prompt 时就不会发生任何求值。
...
你也可以把一个或多个 Pandas DataFrame 传给 `create_bodo_dataframes_agent`。这些 DataFrame 会在传给 agent 之前先转换为 Bodo。
