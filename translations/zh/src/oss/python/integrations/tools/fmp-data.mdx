---
title: "Fmp data 集成"
description: "使用 LangChain Python 集成 Fmp data 工具。"
---
...
FMP（Financial Modeling Prep）的 LangChain 集成提供了一种无缝方式，让你通过自然语言查询访问金融市场数据。该集成包含两个主要组件：

- `FMPDataToolkit`: 基于自然语言查询创建工具集合
- `FMPDataTool`: 一个统一的单工具，会自动选择并使用合适的 endpoints

该集成利用 LangChain 的语义搜索能力，将用户查询与最相关的 FMP API endpoint 进行匹配，让金融数据访问更直观、更高效。
...
你可以像使用其他工具一样，通过创建包含目标模型的 chain 来链式调用该工具。
