---
title: "Vectara 集成"
description: "使用 LangChain Python 集成 Vectara 工具。"
---

[Vectara](https://vectara.com/) 是一个值得信赖的 AI Assistant 与 Agent 平台，重点面向任务关键型应用的企业级就绪能力。更多信息可参阅 [details](../providers/vectara.ipynb)。

[Vectara](https://vectara.com/) 提供了多个可与 LangChain 一起使用的工具。

- **VectaraSearch**：用于在你的 corpus 上进行语义搜索
- **VectaraRAG**：用于通过 RAG 生成摘要
- **VectaraIngest**：用于把文档导入 corpus
- **VectaraAddFiles**：用于上传文件

## 设置

要使用 `Vectara Tools`，首先需要安装 partner package。
...
1. 如果你还没有账户，请前往 [Sign up](https://www.vectara.com/integrations/langchain) 注册免费的 Vectara 试用。
2. 在账户中，你可以创建一个或多个 corpus。每个 corpus 表示一个在导入输入文档后保存文本数据的区域。要创建 corpus，请点击 **"Create Corpus"** 按钮。然后为 corpus 提供名称和描述。你也可以选择定义过滤属性并应用一些高级选项。点击已创建的 corpus 后，可以在顶部直接看到它的名称和 corpus ID。
3. 接下来你需要创建 API key 来访问 corpus。请在 corpus 视图中点击 **"Access Control"** 标签，再点击 **"Create API Key"** 按钮。为密钥命名，并选择该密钥是 query-only 还是 query+index。点击 "Create" 后，这个 API key 就会生效。请妥善保管，不要泄露。

要将 LangChain 与 Vectara 一起使用，你需要两个值：`corpus_key` 和 `api_key`。
你可以通过两种方式向 LangChain 提供 `VECTARA_API_KEY`：

## 实例化

1. 在环境中设置 `VECTARA_API_KEY`。

   例如，可以像下面这样用 `os.environ` 和 `getpass` 设置：
...
2. 把它传给 `Vectara` vectorstore 构造函数：
...
首先，我们把 state-of-the-union 文本加载到 Vectara 中。

注意，这里使用的是 `VectaraAddFiles` 工具，它不需要任何本地处理或切分。Vectara 会接收文件内容，并完成所有必要的预处理、切分和 embedding，然后存入其知识库。

这里使用的是一个 `.txt` 文件，但同样适用于许多其他 [file types](https://docs.vectara.com/docs/api-reference/indexing-apis/file-upload/file-upload-filetypes)。
...
现在我们创建一个 `VectaraQueryConfig` 对象，用来控制检索和摘要生成选项：
- 启用摘要生成，并指定希望 LLM 从前 7 个最匹配的 chunk 中挑选内容，并用英文作答

基于这个配置，我们创建一个封装完整 Vectara RAG 流水线的 LangChain 工具 `VectaraRAG`：
...
`VectaraSearch` 工具也可以像 retriever 一样使用。

在这种模式下，它的行为与其他 LangChain retriever 一样。这个模式主要用于语义搜索，因此这里我们关闭摘要生成：
...
你可以把 Vectara 工具与其他 LangChain 组件串联使用。下面的示例展示了如何：

- 为额外处理设置 ChatOpenAI 模型
- 为特定摘要需求创建自定义 prompt template
- 使用 LangChain 的 Runnable 接口把多个组件串起来
- 处理并格式化 Vectara 返回的 JSON 响应
...
下面的代码演示如何使用 Vectara 工具与 LangChain 创建 agent。
...
`VectaraIngest` 工具允许你把文本内容直接导入 Vectara corpus。当你手头已经有文本内容，不想先创建文件时，这会很有用。
...
详情可参阅 Vectara [tools](https://github.com/vectara/langchain-vectara/blob/main/libs/vectara/langchain_vectara/tools.py) 的实现。
