---
title: "Scrapeless crawl 集成"
description: "使用 LangChain Python 集成 Scrapeless crawl 工具。"
---

[**Scrapeless**](https://www.scrapeless.com/) 提供灵活且功能丰富的数据采集服务，支持广泛的参数自定义和多格式导出能力。这些能力使 LangChain 能更有效地集成和利用外部数据。其核心功能模块包括：
...
ScrapelessCrawlerScrapeTool 允许你使用 Scrapeless 的 Crawler Scrape API，从一个或多个网站抓取内容。你可以提取主体内容、控制格式、请求头、等待时间和输出类型。

该工具接受以下参数：

- `urls`（必填，List[str]）：一个或多个你想抓取的网站 URL。
- `formats`（可选，List[str]）：定义抓取输出的格式，默认值为 `['markdown']`。可选项包括：
  - `'markdown'`
  - `'rawHtml'`
  - `'screenshot@fullPage'`
  - `'json'`
  ...
  - `'screenshot'`
  - `'html'`
- `only_main_content`（可选，bool）：是否仅返回页面主体内容，排除页眉、导航、页脚等。默认值为 True。
- `include_tags`（可选，List[str]）：输出中要包含的 HTML 标签列表，例如 `['h1', 'p']`。如果设为 None，则不显式包含任何标签。
- `exclude_tags`（可选，List[str]）：输出中要排除的 HTML 标签列表。如果设为 None，则不显式排除任何标签。
- `headers`（可选，Dict[str, str]）：请求时附带的自定义请求头，例如 cookies 或 user-agent。默认值为 None。
- `wait_for`（可选，int）：抓取前等待的毫秒数。适合用于等待页面完全加载。默认值为 `0`。
- `timeout`（可选，int）：请求超时时间，单位为毫秒。默认值为 `30000`。

### ScrapelessCrawlerCrawlTool

ScrapelessCrawlerCrawlTool 允许你使用 Scrapeless 的 Crawler Crawl API，从基础 URL 开始爬取网站。它支持 URL 高级过滤、爬取深度控制、内容抓取选项、自定义请求头等能力。

该工具接受以下参数：

- `url`（必填，str）：开始爬取的基础 URL。

- `limit`（可选，int）：最大爬取页面数。默认值为 `10000`。
- `include_paths`（可选，List[str]）：用于包含 URL 的 pathname 正则表达式模式。只有匹配这些模式的 URL 会被纳入爬取。例如设置为 `["blog/.*"]` 时，只会包含 `/blog/` 路径下的 URL。默认值为 None。
- `exclude_paths`（可选，List[str]）：用于排除 URL 的 pathname 正则表达式模式。例如设置为 `["blog/.*"]` 时，会排除 `/blog/` 路径下的 URL。默认值为 None。
- `max_depth`（可选，int）：相对于基础 URL 的最大爬取深度，以 URL 路径中的斜杠数量来衡量。默认值为 `10`。
- `max_discovery_depth`（可选，int）：基于发现顺序的最大爬取深度。根页面和 sitemap 中的页面深度为 `0`。例如设置为 `1` 并忽略 sitemap 时，只会爬取输入 URL 及其直接链接。默认值为 None。
- `ignore_sitemap`（可选，bool）：是否在爬取时忽略网站 sitemap。默认值为 False。
- `ignore_query_params`（可选，bool）：是否忽略查询参数差异，以避免重复抓取相似 URL。默认值为 False。
- `deduplicate_similar_urls`（可选，bool）：是否去重相似 URL。默认值为 True。
- `regex_on_full_url`（可选，bool）：正则匹配是否应用于完整 URL，而不仅是路径。默认值为 True。
- `allow_backward_links`（可选，bool）：是否允许爬取 URL 层级之外的回链。默认值为 False。
- `allow_external_links`（可选，bool）：是否允许爬取指向外部网站的链接。默认值为 False。
- `delay`（可选，int）：页面抓取之间的延迟秒数，用于遵守速率限制。默认值为 `1`。
- `formats`（可选，List[str]）：抓取内容的格式，默认值为 `["markdown"]`。可选项包括：
  - `'markdown'`
  - `'rawHtml'`
  - `'screenshot@fullPage'`
  - `'json'`
  ...
  - `'screenshot'`
  - `'html'`
- `only_main_content`（可选，bool）：是否仅返回主体内容，排除页眉、导航栏、页脚等。默认值为 True。
- `include_tags`（可选，List[str]）：输出中要包含的 HTML 标签列表，例如 `['h1', 'p']`。默认值为 None，表示不显式包含过滤。
- `exclude_tags`（可选，List[str]）：输出中要排除的 HTML 标签列表。默认值为 None，表示不显式排除过滤。
- `headers`（可选，Dict[str, str]）：请求时附带的自定义 HTTP 头，例如 cookies 或 user-agent 字符串。默认值为 None。
- `wait_for`（可选，int）：抓取内容前等待的毫秒数，用于等待页面完全加载。默认值为 `0`。
- `timeout`（可选，int）：请求超时时间，单位为毫秒。默认值为 `30000`。
