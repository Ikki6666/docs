...
description: "使用 LangChain Python 集成 Tavily crawl 工具。"
---

[Tavily](https://tavily.com) 是专为 AI agent（LLM）构建的搜索引擎，能够快速提供实时、准确且基于事实的结果。Tavily 提供 [Crawl](https://docs.tavily.com/documentation/api-reference/endpoint/crawl) endpoint，可从起始 URL 发起结构化网页遍历，并内置内容提取与智能发现能力。
...
| :---: | :---: |:--------------------------------------------------------:| :---: |
| ❌ | ✅ | base URL, page URL, raw content, images, favicon | 每月 1,000 免费积分 |
...
你还需要设置 Tavily API key。可以访问[这个网站](https://app.tavily.com/sign-in)创建账户并获取 API key。
...
- `max_depth`（可选，int）：从起始 URL 开始的最大跳数。默认为 1。
- `max_breadth`（可选，int）：每一层最多抓取的页面数量。默认为 20。
- `limit`（可选，int）：抓取页面总数上限。默认为 50。
- `instructions`（可选，str）：用于引导爬虫的自然语言指令。
- `select_paths`（可选，list[str]）：仅抓取 URL 中包含这些路径正则的页面。
- `select_domains`（可选，list[str]）：仅抓取这些域名正则匹配的页面。
- `exclude_paths`（可选，list[str]）：跳过 URL 中包含这些路径正则的页面。
- `exclude_domains`（可选，list[str]）：跳过这些域名正则匹配的页面。
- `allow_external`（可选，bool）：允许爬虫跟随外部链接。
- `extract_depth`（可选，str）：每个抓取页面的提取深度，可选 `"basic"` 或 `"advanced"`。
- `format`（可选，str）：内容格式，可选 `"markdown"` 或 `"text"`。
- `include_images`（可选，bool）：是否包含在每个页面上发现的图片。

有关可用参数的完整说明，请参阅 [Tavily Crawl API documentation](https://docs.tavily.com/documentation/api-reference/endpoint/crawl)。
...
- `url`（必填）：开始抓取的基础 URL。
- 调用时还可以设置以下参数：`instructions`、`select_paths`、`select_domains`、`exclude_paths`、`exclude_domains`、`allow_external`。

注意：这些可选参数也可供 agent 动态设置。如果你在实例化时设置了某个参数，然后在调用工具时传入不同的值，工具会使用调用时传入的值。
...
你也可以使用模型生成的 ToolCall 来调用该工具，此时会返回一个 ToolMessage：
...
可以将 crawl 工具绑定到模型上，直接在 agent 中使用。这样 agent 就能动态设置可用参数，以控制抓取的位置和深度。
...
有关全部 Tavily Crawl API 特性和配置的详细文档，请参阅 API reference：[docs.tavily.com/documentation/api-reference/endpoint/crawl](https://docs.tavily.com/documentation/api-reference/endpoint/crawl)
