...
PrologTool 类可用于生成 LangChain 工具，这些工具会使用 Prolog 规则来生成答案。
...
然后再把这个结果传回给 LLM，LLM 会基于工具响应回答原始查询：
...
要在 agent 中使用 Prolog 工具，请将其传给 agent 构造函数：
...
详见 [langchain-prolog.readthedocs.io/en/latest/modules.html](https://langchain-prolog.readthedocs.io/en/latest/modules.html)。
