...
description: "使用 LangChain Python 集成 IBM watsonx.ai SQL 工具。"
---

本示例展示如何使用 `langchain-ibm` 的 `watsonx.ai` SQL Database Toolkit，它基于 Flight service。

<Warning>
构建面向 SQL 数据库的问答系统需要执行模型生成的 SQL 查询，这天然带有安全风险。请确保数据库连接权限始终尽可能严格地限制在 agent 的实际需求范围内。这能降低风险，但无法完全消除由模型驱动系统带来的风险。
</Warning>
...
要访问 `langchain-ibm` SQL Database toolkit，你需要创建一个 IBM watsonx.ai 账户、获取 API key，并安装 `langchain-ibm` 集成包。

### Credentials

该单元定义了使用 watsonx SQL Database Toolkit 所需的 WML 凭证。

**Action:** 提供 IBM Cloud 用户 API key。详情请参阅
...
要设置 SQL Database toolkit，首先必须实例化 `WatsonxSQLDatabase` 类。该类会通过 Flight SQL client 从 IBM watsonx.ai 数据库连接资产中获取所需信息。有关以编程方式创建 Database connection Asset 的更多细节，请参阅 watsonx.ai Python SDK [documentation](https://ibm.github.io/watsonx-ai-python-sdk/v1.4.7/autoai_working_with_dataconnection.html#connection-asset-databaselocation)。
...
在某些场景下，你还可以选择把 IBM 的 [`APIClient`](https://ibm.github.io/watsonx-ai-python-sdk/base.html#apiclient) 对象传入 `WatsonxSQLDatabase` 类。
...
有关 `WatsonxSQLDatabaseToolkit` 全部功能和配置的详细文档，请参阅 [API reference](https://reference.langchain.com/python/integrations/langchain_ibm/WatsonxSQLDatabaseToolkit/)。
