...
[CAMB AI](https://camb.ai) 提供多语言音频与本地化服务，支持 140 多种语言，包括文本转语音、翻译、转录、语音克隆和音频生成。

## 概览

### 集成详情

| Class | Package | Serializable | [JS 支持](https://js.langchain.com/docs/integrations/tools) |  Version |
|:------|:--------|:------------:|:--------------------------------------------------------------:|:--------:|
...
|:-----|:------------|:--------|
| `CambTTSTool` | 文本转语音，将文本转换为自然语音 | 音频文件路径、base64 或字节数据 |
| `CambTranslatedTTSTool` | 一步完成文本翻译并转换为语音 | 音频文件路径、base64 或字节数据 |
| `CambTranslationTool` | 在 140 多种语言之间进行文本翻译 | 翻译后的文本 |
| `CambTranscriptionTool` | 带说话人识别的语音转文本 | 包含文本和分段的 JSON |
| `CambVoiceListTool` | 列出可用于 TTS 的语音 | 语音列表 JSON |
...
要访问 CAMB AI 工具，需要创建一个 CAMB AI 账户，并从 [camb.ai](https://camb.ai) 获取 API 密钥。
...
同时建议配置 LangSmith，以便为工具调用提供一流的可观测性/<Tooltip tip="Log each step of a model's execution to debug and improve it">追踪</Tooltip>。要启用自动追踪，请设置你的 [LangSmith](/langsmith/observability) API 密钥：
...
你可以使用 `CambToolkit` 一次性获取全部工具，也可以单独实例化某个工具。
...
在 140 多种语言之间翻译文本，并可选控制正式程度：

```python
from langchain_camb import CambTranslationTool

# 语言代码（完整列表见下方 Language codes 部分）
LANGUAGES = {
    "english": 1,
    "spanish": 54,
    "french": 76,
...
转录音频、翻译内容，并生成另一种语言的语音，这是配音工作流的基础：
...
你可以将 CAMB AI 工具与 LangGraph agent 结合，创建强大的多语言 AI 助手：
...
CAMB AI 在翻译和转录中使用整数语言代码。常见代码如下：
...
对于 TTS，请使用 `"en-us"`、`"es-es"`、`"fr-fr"` 这样的 BCP-47 代码。

---

## API reference

有关 CAMB AI 全部特性和配置的详细文档，请参阅 [CAMB AI API reference](https://docs.camb.ai)。
