...
description: "使用 LangChain Python 集成 ScrapeGraph 工具。"
---

本指南提供一个快速概览，帮助你开始使用 ScrapeGraph [tools](/oss/integrations/tools/)。如需了解 ScrapeGraph 的全部特性与配置详情，请参阅 [API reference](https://python.langchain.com/docs/integrations/tools/scrapegraph)。
...
| `SmartCrawlerTool` | 通过爬取从多个页面提取数据 | URL + prompt + crawl options | JSON |
| `MarkdownifyTool` | 将网页转换为 markdown | URL | Markdown text |
| `GetCreditsTool` | 检查 API credits | None | Credit info |
...
你需要一个 ScrapeGraph AI API key 才能使用这些工具。可前往 [scrapegraphai.com](https://scrapegraphai.com) 获取。
...
SmartCrawlerTool 允许你从某个网站爬取多个页面，并通过高级爬取选项（如深度控制、页面数量限制、域名限制）提取结构化数据。
...
如需了解 ScrapeGraph 全部特性与配置的详细文档，请参阅 [LangChain API reference](https://python.langchain.com/docs/integrations/tools/scrapegraph)。

或者查看[官方 SDK 仓库](https://github.com/ScrapeGraphAI/langchain-scrapegraph)。
