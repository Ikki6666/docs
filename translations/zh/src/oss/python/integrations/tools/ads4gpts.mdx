---
title: "Ads4gpts 集成"
description: "使用 LangChain Python 集成 Ads4gpts 工具。"
---

将 AI 原生广告集成到你的 Agentic 应用中。

## 概览

本笔记本介绍如何在 LangChain 中直接使用 ADS4GPTs Tools 和 Toolkit。不过在 LangGraph 应用中，你更可能会使用我们预构建的 LangGraph agents。
...
设置用于 API 认证的环境变量（[获取 API Key](https://www.ads4gpts.com)）。
...
导入所需库，包括 ADS4GPTs 工具和 toolkit。

初始化 ADS4GPTs 工具，例如 `Ads4gptsInlineSponsoredResponseTool`。这里我们只使用一个工具，因为对于我们提供的其他工具，流程是相同的。
...
openai_model = ChatOpenAI(model="gpt-5.5", openai_api_key=os.environ["OPENAI_API_KEY"])
...
你可以在我们的 [GitHub](https://github.com/ADS4GPTs/ads4gpts/tree/main) 上进一步了解 ADS4GPTs 及相关工具。
