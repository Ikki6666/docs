---
title: "LinkupSearchTool 集成"
description: "使用 LangChain Python 与 LinkupSearchTool tool 集成。"
---

> [Linkup](https://www.linkup.so/) 提供了一个 API，用于把 LLM 连接到 Web 和 Linkup Premium Partner 数据源。

本指南提供了一个快速概览，帮助你开始使用 `LinkupSearchTool` [tool](/oss/langchain/tools)。
...
| :--- | :--- | :---: | :---: | :---: |
| `LinkupSearchTool` | `langchain-linkup` | ❌ | ❌ |  ![PyPI - Version](https://img.shields.io/pypi/v/langchain-linkup?style=flat-square&label=%20) |

## 设置

要使用 Linkup provider，你需要一个有效的 API key，可以通过[注册](https://app.linkup.so/sign-up)获取。运行下面的示例时，你还需要一个 OpenAI API key。

### 安装

这个工具位于 `langchain-linkup` 包中：
...
另外，最好也配置一下 [LangSmith](https://smith.langchain.com)，虽然不是必须，但它能提供一流的可观测性：
...
我们可以先把工具绑定到一个 [tool-calling model](/oss/langchain/tools/)，然后调用它，把它接入 chain：
