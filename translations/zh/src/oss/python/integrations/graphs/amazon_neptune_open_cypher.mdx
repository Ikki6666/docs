---
title: "Amazon Neptune openCypher 集成"
description: "使用 LangChain Python 与 Amazon Neptune openCypher graph 集成。"
---

>[Amazon Neptune](https://aws.amazon.com/neptune/) 是一个高性能的 graph analytics 和 serverless database，具备更强的可扩展性和可用性。
>
>本示例展示如何使用 `openCypher` 查询 `Neptune` graph database，并返回可读性强的人类响应。
>
>[Cypher](https://en.wikipedia.org/wiki/Cypher_(query_language)) 是一种声明式 graph query language，可对 property graph 进行高效且富有表现力的数据查询。
>
