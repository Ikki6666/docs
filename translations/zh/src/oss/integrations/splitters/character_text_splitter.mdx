---
title: "按字符分割 - Text splitter integration guide"
---

基于字符的分割是最简单的文本分割方式。它使用指定的字符序列来切分文本（默认值：`"\n\n"`），并用字符数来衡量块长度。

**要点**：
1. **文本如何被分割**：按给定的字符分隔符分割。
2. **块大小如何衡量**：按字符数衡量。

:::python
你可以在以下两种方式之间选择：
- `.split_text`：返回普通字符串块。
- `.create_documents`：返回 LangChain @[Document] 对象，在下游任务需要保留元数据时很有用。
:::
:::js
你可以在以下两种方式之间选择：
- `.splitText`：返回普通字符串块。
- `.createDocuments`：返回 LangChain @[Document] 对象，在下游任务需要保留元数据时很有用。
:::
...
:::python
使用 `.create_documents` 可以把与每个文档关联的元数据传递到输出块中：
...
:::
:::js
使用 `.createDocuments` 可以把与每个文档关联的元数据传递到输出块中：
