---
title: "ChatOllama 集成"
description: "使用 LangChain JavaScript 集成 ChatOllama 聊天模型。"
---

[Ollama](https://ollama.ai/) 允许你在本地运行开源大型语言模型（LLM），例如 Llama 3.1。

Ollama 将模型权重、配置和数据打包到一个由 Modelfile 定义的统一包中。它还会优化包括 GPU 使用在内的安装与配置细节。

本文将帮助你开始使用 `ChatOllama` [chat models](/oss/langchain/models)。如需查看 `ChatOllama` 全部功能与配置的详细文档，请前往 [API reference](https://reference.langchain.com/javascript/langchain-ollama/ChatOllama)。

## 概览

### 集成详情

Ollama 允许你使用能力各异的大量模型。下面详情表中的部分字段仅适用于 Ollama 提供的某些模型子集。

完整的受支持模型及其变体列表，请参见 [Ollama model library](https://ollama.com/search) 并按 tag 搜索。
...
按照[这些说明](https://github.com/ollama/ollama)设置并运行本地 Ollama 实例。然后，安装 `@langchain/ollama` 包。
...
Ollama 现在已为其[部分可用模型](https://ollama.com/search?c=tools)提供原生 tool calling 支持。下面的示例演示了如何从 Ollama 模型中调用工具。
...
Ollama 原生支持[结构化输出](https://docs.ollama.com/capabilities/structured-outputs)对所有模型生效，这使你可以通过调用 `.withStructuredOutput()` 强制模型返回特定格式。
...
如果你更希望通过 tool calling 使用结构化输出，可以传入 `method: "functionCalling"` 选项：
...
Ollama 从 0.1.15 版本开始支持 [LLaVA](https://ollama.ai/library/llava) 等开源多模态模型。
你可以把图片作为消息 `content` 字段的一部分，传递给这类[支持多模态](/oss/langchain/messages#multimodal)的模型：
...
如需查看 `ChatOllama` 全部功能与配置的详细文档，请前往 [API reference](https://reference.langchain.com/javascript/langchain-ollama/ChatOllama)。
