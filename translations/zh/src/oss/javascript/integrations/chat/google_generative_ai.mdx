...
description: "使用 LangChain JavaScript 集成 ChatGoogleGenerativeAI 聊天模型。"
---

[Google AI](https://ai.google.dev/) 提供了多种聊天模型，其中包括强大的 Gemini 系列。有关最新模型、特性、上下文窗口等信息，请参见 [Google AI docs](https://ai.google.dev/gemini-api/docs/models/gemini)。

本文将帮助你开始使用 `ChatGoogleGenerativeAI` [chat models](/oss/langchain/models)。如需查看 `ChatGoogleGenerativeAI` 全部功能与配置的详细文档，请前往 [API reference](https://reference.langchain.com/javascript/langchain-google-genai/ChatGoogleGenerativeAI)。

<Note>
    **该库将被弃用**

    该库基于 Google 的一个已弃用库构建，
    将被 [ChatGoogle](/oss/integrations/chat/google) 库替代。
...
你可以通过 `@langchain/google-genai` 集成包中的 `ChatGoogleGenerativeAI` 类，在 LangChain 中访问 Google 的 `gemini`、`gemini-vision` 模型，以及其他生成式模型。


<Tip>
你也可以通过 LangChain 的 VertexAI 和 VertexAI-web 集成访问 Google 的 `gemini` 系列模型。请参见 [Vertex AI integration docs](/oss/integrations/chat/google_vertex_ai)。
</Tip>

### 凭证

在这里获取 API key：[https://ai.google.dev/tutorials/setup](https://ai.google.dev/tutorials/setup)
...
Gemini 模型具有默认安全设置，这些设置可以被覆盖。如果你的模型经常返回 “Safety Warnings”，可以尝试调整模型的 `safetySettings` 属性。例如，要关闭对危险内容的安全阻断，你可以从 `@google/generative-ai` 包导入枚举，然后像下面这样构造 LLM：
...
Google AI 的工具调用在大多数情况下[与其他模型的工具调用](/oss/langchain/tools)类似，但在 schema 上有一些限制。

Google AI API 不允许工具 schema 中包含具有未知属性的对象。例如，下面的 Zod schema 会抛出错误：
...
Google 还提供了一个内置搜索工具，你可以用它把内容生成 grounding 到真实世界信息中。下面演示如何使用它：
...
Google Generative AI 还支持代码执行。借助内置的 `CodeExecutionTool`，你可以让模型生成代码、执行代码，并在最终补全中使用执行结果：
...
上下文缓存允许你把某些内容只传给模型一次，缓存输入 token，并在后续请求中引用这些缓存 token 以降低成本。你可以通过 `GoogleAICacheManager` 类创建 `CachedContent` 对象，然后通过 `enableCachedContent()` 方法把这个 `CachedContent` 对象传给你的 `ChatGoogleGenerativeAIModel`。
...
- 上下文缓存的最小输入 token 数是 32,768，最大值与对应模型的最大值一致。

## Gemini 提示常见问题

在本文写作时（2023/12/12），Gemini 对它接受的提示类型和结构有一些限制。具体来说：

1. 当提供多模态（图片）输入时，最多只能有 1 条 `"human"`（用户）类型消息。你不能传入多条消息（不过这唯一的一条 human 消息可以有多个内容条目）
2. 原生不支持 system 消息，如果存在 system 消息，它会被合并进第一条 human 消息中
3. 对于常规聊天对话，消息必须遵循 human/ai/human/ai 的交替模式。不能连续提供 2 条 AI 或 human 消息
4. 如果消息违反 LLM 的安全检查，可能会被阻断。这种情况下，模型会返回空响应

---

## API reference

如需查看 `ChatGoogleGenerativeAI` 全部功能与配置的详细文档，请前往 [API reference](https://reference.langchain.com/javascript/langchain-google-genai/ChatGoogleGenerativeAI)。
