...
description: "使用 LangChain JavaScript 集成 Jigsawstack tool。"
---

JigsawStack Tool 为你的 agent 提供以下能力：

- `JigsawStackAIScrape`：使用高级 AI 抓取网页内容。
- `JigsawStackAISearch`：执行 AI 驱动的网页搜索，并返回高质量结果。
- `JigsawStackSpeechToText`：使用 Whisper large V3 AI 模型转录视频和音频文件。
- `JigsawStackVOCR`：使用 prompt 识别、描述并提取图像中的数据。
- `JigsawStackTextToSQL`：根据文本生成语义正确的 SQL 查询。

## 设置

- 配置一个 [account](https://jigsawstack.com/dashboard) ，可免费开始
