---
title: "ExaSearchResults 集成"
description: "使用 LangChain JavaScript 集成 ExaSearchResults tool。"
---

Exa，前身为 Metaphor Search，是一个专为 LLM 使用而设计的搜索引擎。你可以通过自然语言查询在互联网上搜索文档，然后从目标文档中提取清洗后的 HTML 内容。

与基于关键词的搜索，Google，不同，Exa 的神经搜索能力能够在语义层面理解查询并返回相关文档。例如，我们可以搜索 `"fascinating article about cats"`，然后对比 Google 和 Exa 的搜索结果。Google 给出的是围绕关键词 “fascinating” 做 SEO 优化的列表文章，而 Exa 直接能返回真正相关的结果。

本页介绍如何在 LangChain 中使用 `ExaSearchResults`。
...
| :--- | :--- | :---: | :---: | :---: |
| [`ExaSearchResults`](https://reference.langchain.com/javascript/langchain-exa/ExaSearchResults) | [`@langchain/exa`](https://npmjs.com/package/@langchain/exa) | ❌ | ✅ |  ![NPM - Version](https://img.shields.io/npm/v/@langchain/exa?style=flat-square&label=%20&) |
...
首先，获取一个 Exa API key，并将其设置为环境变量。你可以通过[注册 Exa](https://dashboard.exa.ai/login)获得每月 1000 次免费搜索。

```bash
export EXASEARCH_API_KEY="your-api-key"
```

同时也建议配置 LangSmith，以获得一流的可观测性，虽然不是必需：
...
### [直接用 args 调用](/oss/langchain/tools)
...
我们可以先把工具绑定到一个[支持 tool calling 的 model](/oss/langchain/tools)上，然后在 chain 中使用它：
...
我们可以创建使用 `ExaRetriever` 和 `createRetrieverTool` 的 LangChain tools。借助这些工具，可以构建一个简单的搜索 agent，回答任意主题的问题。

这里使用 LangGraph 来创建 agent。请确保你已经安装 `@langchain/langgraph`：
...
For more details, you can read the [full Atlantic article](https://www.theatlantic.com/health/archive/2023/10/cat-dental-care-toothbrushing/675123/).
```
...
有关 `ExaSearchResults` 全部功能和配置的详细文档，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-exa/ExaSearchResults)
