---
title: "Web browser 集成"
description: "使用 LangChain JavaScript 集成 Web browser tool。"
---

import LangchainCommunityUnmaintainedJs from '/snippets/oss/langchain-community-unmaintained-js.mdx';

Webbrowser Tool 让你的 agent 能够访问网站并提取信息。它向 agent 暴露的描述是：
...
- 当 Agent 仅传入 URL 调用它时，它会生成网站内容摘要
- 当 Agent 传入 URL 和要查找内容的描述时，它会改用内存中的 Vector Store 找到最相关的片段，并对这些片段进行总结

## 设置

要使用 Webbrowser Tool，你需要安装以下依赖：
