---
title: "JSON 文件集成"
description: "集成 JSON 文件文档加载器以在 LangChain JavaScript 中使用。"
---

JSON 加载器使用 [JSON pointer](https://github.com/janl/node-jsonpointer) 来定位 JSON 文件中你想提取的键。

### 不使用 JSON pointer 的示例

最简单的使用方式是不指定 JSON pointer。
加载器会读取 JSON 对象中找到的所有字符串。
...
你可以选择要从 JSON 对象中的哪些键提取字符串。

在这个示例中，只提取 `"from"` 和 `"surname"` 条目中的信息。
