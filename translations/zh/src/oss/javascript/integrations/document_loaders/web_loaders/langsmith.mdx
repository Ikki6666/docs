...
description: "集成 LangSmithLoader 文档加载器以在 LangChain JavaScript 中使用。"
---


本文档简要介绍如何开始使用 `LangSmithLoader` [文档加载器](/oss/integrations/document_loaders/)。如需查看 `LangSmithLoader` 的全部特性和配置详情，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-core/document_loaders/langsmith/LangSmithLoader)。
...
要访问 LangSmith 文档加载器，需要安装 `@langchain/core`，创建一个 [LangSmith](https://langsmith.com/) 账号，并获取 API key。
...
在这个示例中，我们会创建一个新数据集，并在文档加载器中使用它。
...
如需查看 `LangSmithLoader` 的全部特性和配置详情，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-core/document_loaders/langsmith/LangSmithLoader)
