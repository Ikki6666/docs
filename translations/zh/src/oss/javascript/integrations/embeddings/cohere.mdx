...
description: "集成 CohereEmbeddings embedding model 以在 LangChain JavaScript 中使用。"
---

本文将帮助你开始在 LangChain 中使用 CohereEmbeddings [embedding models](/oss/integrations/embeddings)。如需查看 `CohereEmbeddings` 的特性和配置选项详情，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-cohere/CohereEmbeddings)。

## 概览

### 集成信息

| Class | Package | Local | [Py support](https://python.langchain.com/docs/integrations/embeddings/cohere/) | Downloads | Version |
| :--- | :--- | :---: | :---: |  :---: | :---: |
| [`CohereEmbeddings`](https://reference.langchain.com/javascript/langchain-cohere/CohereEmbeddings) | [`@langchain/cohere`](https://www.npmjs.com/package/@langchain/cohere) | ❌ | ✅ | ![NPM - Downloads](https://img.shields.io/npm/dm/@langchain/cohere?style=flat-square&label=%20&) | ![NPM - Version](https://img.shields.io/npm/v/@langchain/cohere?style=flat-square&label=%20&) |

## 设置

要访问 Cohere embedding models，需要创建 Cohere 账号、获取 API key，并安装 `@langchain/cohere` 集成包。
...
LangChain 的 CohereEmbeddings 集成位于 `@langchain/cohere` 包中：
...
我们可以实例化一个自定义 `CohereClient`，并将其传入 `CohereEmbeddings` 构造函数。
...
如需查看 `CohereEmbeddings` 的全部特性和配置详情，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-cohere/CohereEmbeddings)。
