---
title: "TogetherAIEmbeddings 集成"
description: "集成 TogetherAIEmbeddings embedding model 以在 LangChain JavaScript 中使用。"
---

本文将帮助你开始在 LangChain 中使用 TogetherAIEmbeddings [embedding models](/oss/integrations/embeddings)。如需查看 `TogetherAIEmbeddings` 的特性和配置选项详情，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-together-ai/TogetherAIEmbeddings)。
...
要访问 Together AI embedding models，请创建 Together 账号、获取 API key，并安装 `@langchain/together-ai` 集成包。

### 凭据

你可以[注册 Together 账号并创建 API key](https://api.together.xyz/)。然后设置 `TOGETHER_AI_API_KEY` 环境变量：
...
LangChain 的 TogetherAIEmbeddings 集成位于 `@langchain/together-ai` 包中：
...
如需查看 `TogetherAIEmbeddings` 的全部特性和配置详情，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-together-ai/TogetherAIEmbeddings)。
