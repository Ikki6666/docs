...
description: "集成 MistralAIEmbeddings embedding model 以在 LangChain JavaScript 中使用。"
---

本文将帮助你开始在 LangChain 中使用 MistralAIEmbeddings [embedding models](/oss/integrations/embeddings)。如需查看 `MistralAIEmbeddings` 的特性和配置选项详情，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-mistralai/MistralAIEmbeddings)。
...
要访问 MistralAI embedding models，需要创建 MistralAI 账号、获取 API key，并安装 `@langchain/mistralai` 集成包。
...
LangChain 的 MistralAIEmbeddings 集成位于 `@langchain/mistralai` 包中：
...
Mistral AI 支持三类自定义 hooks，对应三个事件：`beforeRequest`、`requestError` 和 `response`。下面展示了每种 hook 类型的函数签名示例：
...
要把这些 hooks 添加到 embeddings 模型中，可以在构造时直接作为参数传入，它们会被自动添加：
...
`addAllHooksToHttpClient` 方法会先清除当前已添加的所有 hooks，再把更新后的完整 hook 列表重新赋值进去，以避免重复添加 hooks。

你可以逐个移除 hooks，也可以一次性清空模型上的所有 hooks。
...
如需查看 `MistralAIEmbeddings` 的全部特性和配置详情，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-mistralai/MistralAIEmbeddings)。
