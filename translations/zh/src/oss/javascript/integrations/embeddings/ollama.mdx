...
description: "集成 OllamaEmbeddings embedding model 以在 LangChain JavaScript 中使用。"
---

本文将帮助你开始在 LangChain 中使用 Ollama [embedding models](/oss/integrations/embeddings)。如需查看 `OllamaEmbeddings` 的特性和配置选项详情，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-ollama/OllamaEmbeddings)。
...
要访问 Ollama embedding models，请按照[这些说明](https://github.com/ollama/ollama)安装 Ollama，然后安装 `@langchain/ollama` 集成包。
...
LangChain 的 OllamaEmbeddings 集成位于 `@langchain/ollama` 包中：
...
Ollama 的[模型参数](https://github.com/jmorganca/ollama/blob/main/docs/modelfile.md#valid-parameters-and-values)也同样受支持：
...
如需查看 `OllamaEmbeddings` 的全部特性和配置详情，请参阅 [API reference](https://reference.langchain.com/javascript/langchain-ollama/OllamaEmbeddings)
