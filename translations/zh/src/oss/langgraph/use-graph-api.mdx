...
@[StateGraph] 在初始化时接受 @[`state_schema`] 参数，该参数指定图中的节点可以访问和更新的状态的“形状”。
...
节点的并行执行对于加速整体图操作至关重要。 LangGraph 提供对节点并行执行的本机支持，这可以显着增强基于图的工作流的性能。这种并行化是通过扇出和扇入机制实现的，同时利用标准边缘和 @[conditional_edges][add_conditional_edges]。下面是一些示例，展示了如何添加创建适合您的分支数据流。
...
将控制流（边）和状态更新（节点）结合起来非常有用。例如，您可能希望既执行状态更新又决定在同一节点中下一个转到哪个节点。 LangGraph 提供了一种通过从节点函数返回 @[Command] 对象来实现此目的的方法：
